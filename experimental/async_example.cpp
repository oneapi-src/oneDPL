...
main()
{
    ...
    const int n = 100;
    {
        ...
        auto res_1a = dpl::experimental::copy_async(my_policy, dpl::counting_iterator<int>(0),
                                                    dpl::counting_iterator<int>(n), dpl::begin(x));
        //std::iota(x.begin(), x.end(), 0); // x = [0..n]
        auto res_1b = dpl::experimental::fill_async(my_policy, dpl::begin(y), dpl::end(y), 7); // y = [7..7]

        auto res_2a = dpl::experimental::for_each_async(
            my_policy, dpl::begin(x), dpl::end(x), [](auto& e) { ++e; }, res_1a); // x = [1..n]
        auto res_2b = dpl::experimental::transform_async(
            my_policy, dpl::begin(y), dpl::end(y), dpl::begin(y), [](const auto& e) { return e / 2; },
            res_1b); // y = [3..3]

        sycl::buffer<int> z{n}; //std::vector<int> z(n);
        auto res_3 =
            dpl::experimental::transform_async(my_policy, dpl::begin(x), dpl::end(x), dpl::begin(y), dpl::begin(z),
                                               std::plus<int>(), res_2a, res_2b); // z = [4..n+3]

        auto res_4 = dpl::experimental::reduce_async(my_policy, dpl::begin(x), dpl::end(x), 0, std::plus<int>(),
                                                     res_2a); // alpha = n*(n+1)/2
...
#else
        auto r_beta = dpl::experimental::transform_reduce_async(my_policy, dpl::begin(z), dpl::end(z), 0,
                                                                std::plus<int>(), [=](auto e) { return alpha * e; });
