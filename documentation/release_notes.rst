...
New Features
------------
-  Added the Range-based versions of the following algorithms: ``all_of``, ``any_of``, ``count``, ``count_if``, ``equal``,
``move``, ``remove``, ``remove_if``, ``replace``, ``replace_if``. Added the following utility viewes: ``generate``, ``fill``, ``rotate``. 
