...
TYPED_TEST(Drop, largeDropOfAllButOneHasSameSegmentAndRank) {
  ...
  auto drop_view_segments = experimental::dr::ranges::segments(drop_view_result);
  ...
  EXPECT_EQ(experimental::dr::ranges::rank(drop_view_segments[0]),
...
TYPED_TEST(Drop, dropOfFirstSegementHasSameSegmentsSize) {
  ...
  auto drop_view_segments = experimental::dr::ranges::segments(drop_view_result);
  EXPECT_EQ(rng::size(drop_view_segments), experimental::dr::ranges::segments(dv).size() - 1);
}
...
TYPED_TEST(Drop, dropOfOneElementHasAllSegmentsWithSameRanks) {
  ...
  for (std::size_t i = 0; i < rng::size(dv_segments); ++i)
    EXPECT_EQ(experimental::dr::ranges::rank(dv_segments[i]),
              experimental::dr::ranges::rank(drop_view_segments[i]));
}
