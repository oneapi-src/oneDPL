...
TYPED_TEST(Counted, countedOfOneElementHasOneSegmentAndSameRank) {
  ...
  auto counted_view_segments = oneapi::dpl::experimental::dr::ranges::segments(counted_view_result);
  ...
  EXPECT_EQ(oneapi::dpl::experimental::dr::ranges::rank(counted_view_segments[0]),
...
TYPED_TEST(Counted, countedOfFirstSegementHasOneSegmentAndSameRank) {
  ...
  assert(oneapi::dpl::experimental::dr::ranges::segments(dv)[0].size() > bias);
  auto counted_view_result =
      xhp::views::counted(dv.begin() + bias, first_seg_size - bias);
  auto counted_view_segments = oneapi::dpl::experimental::dr::ranges::segments(counted_view_result);
  EXPECT_EQ(rng::size(counted_view_segments), 1);
  EXPECT_EQ(oneapi::dpl::experimental::dr::ranges::rank(counted_view_segments[0]),
            oneapi::dpl::experimental::dr::ranges::rank(oneapi::dpl::experimental::dr::ranges::segments(dv)[0]));
}
...
TYPED_TEST(Counted, countedOfAllButOneSizeHasAllSegmentsWithSameRanks) {
  ...
  for (std::size_t i = 0; i < rng::size(dv_segments); ++i)
    EXPECT_EQ(oneapi::dpl::experimental::dr::ranges::rank(dv_segments[i]),
              oneapi::dpl::experimental::dr::ranges::rank(counted_view_segments[i]));
}
