...
TYPED_TEST(Take, takeOfOneElementHasOneSegmentAndSameRank) {
  ...
  auto take_view_segments = oneapi::dpl::experimental::dr::ranges::segments(take_view_result);
  ...
  EXPECT_EQ(oneapi::dpl::experimental::dr::ranges::rank(take_view_segments[0]),
            oneapi::dpl::experimental::dr::ranges::rank(dv_segments[0]));
}
...
void takeHasSameSegments(std::size_t dv_size, std::size_t take_size) {
  ...
  for (std::size_t i = 0; i < rng::size(dv_segments); ++i)
    EXPECT_EQ(oneapi::dpl::experimental::dr::ranges::rank(dv_segments[i]),
              oneapi::dpl::experimental::dr::ranges::rank(take_view_segments[i]));
}
