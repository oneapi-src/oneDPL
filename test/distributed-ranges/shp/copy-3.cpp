...
TYPED_TEST(CopyTest, tests_from_this_file_run_on_3_devices) {
  EXPECT_EQ(oneapi::dpl::experimental::dr::shp::nprocs(), 3);
  EXPECT_EQ(rng::size(oneapi::dpl::experimental::dr::shp::devices()), 3);
}
...
TYPED_TEST(CopyTest, dist2local_wholesegment) {
  ...
  auto ret_it = oneapi::dpl::experimental::dr::shp::copy(rng::begin(dist_vec) + 4,
