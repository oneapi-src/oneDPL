...
TYPED_TEST(CopyTest, dist2local_async) {
  ...
  oneapi::dpl::experimental::dr::shp::copy_async(rng::begin(dist_vec), rng::end(dist_vec),
                      rng::begin(local_vec))
      .wait();
...
TYPED_TEST(CopyTest, local2dist_async) {
  ...
  oneapi::dpl::experimental::dr::shp::copy_async(rng::begin(local_vec), rng::end(local_vec),
                      rng::begin(dist_vec))
      .wait();
...
TYPED_TEST(CopyTest, dist2local_sync) {
  ...
  auto ret_it = oneapi::dpl::experimental::dr::shp::copy(rng::begin(dist_vec), rng::end(dist_vec),
                              rng::begin(local_vec));
  EXPECT_TRUE(
...
TYPED_TEST(CopyTest, local2dist_sync) {
  ...
  auto ret_it = oneapi::dpl::experimental::dr::shp::copy(rng::begin(local_vec), rng::end(local_vec),
                              rng::begin(dist_vec));
  EXPECT_TRUE(
...
TYPED_TEST(CopyTest, dist2local_range_sync) {
  const typename TestFixture::DistVec dist_vec = {1, 2, 3, 4, 5};
  typename TestFixture::LocalVec local_vec = {0, 0, 0, 0, 0, 9};
  auto ret_it = oneapi::dpl::experimental::dr::shp::copy(dist_vec, rng::begin(local_vec));
  EXPECT_TRUE(
      equal(local_vec, typename TestFixture::LocalVec{1, 2, 3, 4, 5, 9}));
  EXPECT_EQ(*ret_it, 9);
}
...
TYPED_TEST(CopyTest, local2dist_range_sync) {
  const typename TestFixture::LocalVec local_vec = {1, 2, 3, 4, 5};
  typename TestFixture::DistVec dist_vec = {0, 0, 0, 0, 0, 9};
  auto ret_it = oneapi::dpl::experimental::dr::shp::copy(local_vec, rng::begin(dist_vec));
  EXPECT_TRUE(
      equal(dist_vec, typename TestFixture::LocalVec{1, 2, 3, 4, 5, 9}));
  EXPECT_EQ(*ret_it, 9);
}
...
TYPED_TEST(CopyTest, dist2local_async_can_interleave) {
  ...
  auto event_1 =
      oneapi::dpl::experimental::dr::shp::copy_async(rng::begin(dist_vec) + 0, rng::begin(dist_vec) + 4,
                          rng::begin(local_vec) + 0);
  auto event_2 =
      oneapi::dpl::experimental::dr::shp::copy_async(rng::begin(dist_vec) + 1, rng::begin(dist_vec) + 5,
...
TYPED_TEST(CopyTest, local2dist_async_can_interleave) {
  ...
  typename TestFixture::DistVec dist_vec = {0, 0, 0, 0, 0};
  auto event_1 = oneapi::dpl::experimental::dr::shp::copy_async(
      rng::begin(local_vec_1), rng::end(local_vec_1), rng::begin(dist_vec));
  auto event_2 = oneapi::dpl::experimental::dr::shp::copy_async(
...
TYPED_TEST(CopyTest, dist2local_sliced_bothSides) {
  ...
  oneapi::dpl::experimental::dr::shp::copy(rng::begin(dist_vec) + 1, rng::end(dist_vec) - 1,
                rng::begin(local_vec));
  EXPECT_TRUE(equal(
...
TYPED_TEST(CopyTest, dist2local_sliced_left) {
  ...
  oneapi::dpl::experimental::dr::shp::copy(rng::begin(dist_vec) + 1, rng::end(dist_vec),
                rng::begin(local_vec));
...
TYPED_TEST(CopyTest, dist2local_sliced_right) {
  ...
  oneapi::dpl::experimental::dr::shp::copy(rng::begin(dist_vec), rng::end(dist_vec) - 1,
                rng::begin(local_vec));
  EXPECT_TRUE(equal(
...
TYPED_TEST(CopyTest, local2dist_sliced_bothSides) {
  ...
  oneapi::dpl::experimental::dr::shp::copy(rng::begin(local_vec), rng::end(local_vec),
