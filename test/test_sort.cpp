...
class ParanoidKey;
static bool Equal(const ParanoidKey& x, const ParanoidKey& y);
...
public:
    ...
    ParanoidKey& operator=( const ParanoidKey& k ) {
...
// std::stable_sort() fails in move semantics on paranoid test before VS2015
    ...
    ParanoidKey& operator=(ParanoidKey&& k) {
...
// std::stable_sort() fails in move semantics on paranoid test before VS2015
    ...
    friend bool Equal( const ParanoidKey& x, const ParanoidKey& y );
};
...
public:
    ...
    bool operator()( const ParanoidKey& j, const ParanoidKey& k ) const {
...
static bool Equal(float32_t x, float32_t y ) {return x==y;}

static bool Equal(int32_t x, int32_t y ) {return x==y;}
...
void test_sort( Compare compare, Convert convert ) {
...
int32_t main() {
    ...
    for(int32_t kind=0; kind<2; ++kind ) {
        Stable = kind!=0;
        test_sort<ParanoidKey>(KeyCompare(OddTag()), [](size_t k, size_t val) {return ParanoidKey(k, val, OddTag());});
