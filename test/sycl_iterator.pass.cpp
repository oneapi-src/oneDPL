...
struct test_equal
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        exec.queue().wait_and_throw();
#endif
        EXPECT_TRUE(result, "wrong effect from equal with 3 iterators");
        ...
        exec.queue().wait_and_throw();
#endif
        EXPECT_TRUE(result, "wrong effect from equal with 4 iterators");
    }
};
...
struct test_transform_inclusive_scan
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        auto res1 = std::transform_inclusive_scan(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, last1, first2,
                                                  std::plus<T1>(), std::negate<T1>(), value);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        EXPECT_TRUE(res1 == last2, "wrong effect from scan_1");
        auto host_first2 = get_host_pointer(first2);
        auto ii = value;
        ...
        auto res2 = std::transform_inclusive_scan(make_new_policy<new_kernel_name<Policy, 1>>(exec), first1, last1, first2,
                                                  std::plus<T1>(), std::negate<T1>());
        EXPECT_TRUE(res2 == last2, "wrong effect from scan_2");
        host_first2 = get_host_pointer(first2);
        ii = 0;
        for (int i = 0; i < last2 - first2; ++i)
        {
...
struct test_transform_exclusive_scan
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        auto res1 = std::transform_exclusive_scan(make_new_policy<new_kernel_name<Policy, 2>>(exec), first1, last1, first2,
                                                  T1{}, std::plus<T1>(), std::negate<T1>());
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        EXPECT_TRUE(res1 == last2, "wrong effect from scan_1");
...
struct test_copy_if
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        exec.queue().wait_and_throw();
#endif
        EXPECT_TRUE(res1 == last2, "wrong effect from copy_if_1");
        auto host_first2 = get_host_pointer(first2);
        ...
        exec.queue().wait_and_throw();
#endif
        EXPECT_TRUE(res2 == first2 + (last2 - first2) / 2, "wrong effect from copy_if_2");
        host_first2 = get_host_pointer(first2);
...
struct test_remove
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Size n)
    {
        ...
        exec.queue().wait_and_throw();
#endif
        EXPECT_TRUE(res1 == last - 1, "wrong effect from remove_1");
        host_first1 = get_host_pointer(first);
...
struct test_remove_if
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Size n)
    {
        ...
        exec.queue().wait_and_throw();
#endif
        EXPECT_TRUE(res1 == last - 1, "wrong effect from remove_if_1");
        host_first1 = get_host_pointer(first);
        for (int i = 0; i < res1 - first; ++i)
        {
            auto exp = i + 222;
...
struct test_reverse
{
    ...
    operator()(Policy&& exec, Iterator1 first, Iterator1 last, Size n)
    {
        ...
        for (int i = 0; i < (last - first) && is_correct; ++i)
            is_correct = is_correct && (local_copy[i] == host_first1[i]);

        EXPECT_TRUE(is_correct, "wrong effect from reverse");
    }
};
...
struct test_reverse_copy
{
    ...
    operator()(Policy&& exec, Iterator1 first, Iterator1 last, Iterator1 result_first, Iterator1 result_last, Size n)
    {
        ...
        for (int i = 0; i < n && is_correct; ++i)
            is_correct = is_correct && (local_copy[i] == host_first2[i]);

        EXPECT_TRUE(is_correct, "wrong effect from reverse_copy");
    }
};
...
struct test_rotate
{
    ...
    operator()(Policy&& exec, Iterator1 first, Iterator1 last, Size n)
    {
        ...
        EXPECT_TRUE(is_correct, "wrong effect from rotate");
    }
};
...
struct test_rotate_copy
{
    ...
    operator()(Policy&& exec, Iterator1 first, Iterator1 last, Iterator1 result_first, Iterator1 result_last, Size n)
    {
        ...
        EXPECT_TRUE(is_correct, "wrong effect from rotate_copy");
    }
};
...
main()
{
    ...
    PRINT_DEBUG("test_exclusive_scan");
