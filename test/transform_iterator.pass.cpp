...
void test_simple_copy(size_t buffer_size) {
    ...
    auto sycl_source_begin = dpstd::begin(source_buf);
    auto sycl_result_begin = dpstd::begin(result_buf);

    auto transformation = [](int item) { return item + 1; };

    auto tr_sycl_source_begin = dpstd::make_transform_iterator(sycl_source_begin, transformation);
...
void test_ignore_copy(size_t buffer_size) {
    ...
    auto sycl_source_begin = dpstd::begin(source_buf);
    auto sycl_source_end = dpstd::end(source_buf);
    auto sycl_result_begin = dpstd::begin(result_buf);

    auto transformation = [](int) { return std::ignore; };

    auto tr_sycl_result_begin = dpstd::make_transform_iterator(sycl_result_begin, transformation);
...
void test_multi_transform_copy(size_t buffer_size) {
    ...
    auto tr_sycl_source_begin = dpstd::make_transform_iterator(sycl_source_begin, transformation);
    auto tr2_sycl_source_begin = dpstd::make_transform_iterator(tr_sycl_source_begin, transformation);
    auto tr3_sycl_source_begin = dpstd::make_transform_iterator(tr2_sycl_source_begin, transformation);
