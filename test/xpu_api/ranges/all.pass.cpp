...
struct View : std::ranges::view_base {
  int* globalBuff;
  int start_ = 0;
  explicit View(int* p) noexcept(IsNoexcept): globalBuff(p) {}
  constexpr explicit View(int start, int* p) : start_(start), globalBuff(p) {}
...
struct CopyableView : std::ranges::view_base {
  int* globalBuff;
  int start_ = 0;
  explicit CopyableView(int* p) noexcept(IsNoexcept): globalBuff(p) {}
  ...
  constexpr explicit CopyableView(int start, int* p) noexcept : start_(start), globalBuff(p) {}
  constexpr int* begin() const { return globalBuff + start_; }
  constexpr int* end() const { return globalBuff + 8; }
};
...
struct Range {
  int* globalBuff;
  int start_;
  constexpr explicit Range(int start, int* p) noexcept : start_(start), globalBuff(p) {}
  constexpr int* begin() const { return globalBuff + start_; }
  constexpr int* end() const { return globalBuff + 8; }
};
...
struct BorrowableRange {
  int* globalBuff;
  int start_;
  constexpr explicit BorrowableRange(int start, int* p) noexcept : start_(start), globalBuff(p) {}
  constexpr int* begin() const { return globalBuff + start_; }
  constexpr int* end() const { return globalBuff + 8; }
};
template<>
...
struct RandomAccessRange {
  int* globalBuff;
  constexpr explicit RandomAccessRange(int* p): globalBuff(p) {}
...
constexpr bool test() {
  int globalBuff[8];
  {
    ASSERT_SAME_TYPE(decltype(std::views::all(View<true>(globalBuff))), View<true>);
    //static_assert(noexcept(std::views::all(View<true>())));
    //static_assert(!noexcept(std::views::all(View<false>())));

    auto viewCopy = std::views::all(View<true>(2, globalBuff));
...

  {
    ...
    //static_assert(noexcept(std::views::all(CopyableView<true>(globalBuff))));
    //static_assert(!noexcept(std::views::all(CopyableView<false>())));

    CopyableView<true> view(2, globalBuff);
  ...
  {
    auto own = std::views::all(BorrowableRange(2, globalBuff));
  ...
  {
    auto own = std::views::all(RandomAccessRange(globalBuff));
...
int main(int, char**) {
  ...
  const bool res = kernel_test<class std_all_test>([](){ return test();});
