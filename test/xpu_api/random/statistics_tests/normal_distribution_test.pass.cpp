...
int
statistics_check(int nsamples, ScalarRealType mean, ScalarRealType stddev, const std::vector<ScalarRealType>& samples)
{
    // theoretical moments
    double tM = mean;
...
int
test(sycl::queue& queue, oneapi::dpl::internal::element_type_t<RealType> mean,
     oneapi::dpl::internal::element_type_t<RealType> stddev, int nsamples)
{
...
template <class RealType, class UIntType>
int
test_portion(sycl::queue& queue, oneapi::dpl::internal::element_type_t<RealType> mean,
             oneapi::dpl::internal::element_type_t<RealType> stddev, int nsamples, unsigned int part)
{
...
template <class RealType, class UIntType>
int
test_flag(sycl::queue& queue, oneapi::dpl::internal::element_type_t<RealType> mean,
          oneapi::dpl::internal::element_type_t<RealType> stddev, int nsamples)
{
    ...
    // generation
    {
        ...
        queue.submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for<>(sycl::range<1>(nsamples / (num_elems + 1)), [=](sycl::item<1> idx) {
                unsigned long long offset = idx.get_linear_id() * num_to_skip;
                oneapi::dpl::linear_congruential_engine<UIntType, a, c, m> engine(seed, offset);
                oneapi::dpl::normal_distribution<RealType> distr(mean, stddev);
...
template <class RealType, class UIntType>
int
tests_set_flag(sycl::queue& queue, int nsamples)
{

    // Test for all non-zero parameters
    std::cout << "normal_distribution test<type>, mean = " << 0.0 << ", stddev = " << 1.0 << ", nsamples = " << nsamples
              << ", flag = true, vec_size = " << oneapi::dpl::internal::type_traits_t<RealType>::num_elems;
