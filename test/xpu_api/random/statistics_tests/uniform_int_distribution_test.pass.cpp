...
std::int32_t
statistics_check(int nsamples, IntType left, IntType right, const std::vector<IntType>& samples)
{
    // theoretical moments
    double tM = (left + right - 1.0) / 2.0;
    double tD = ((right - left) * (right - left) - 1.0) / 12.0;
    double tQ = (((right - left) * (right - left)) * ((1.0 / 80.0) * (right - left) * (right - left) - (1.0 / 24.0))) +
                (7.0 / 240.0);
...
int
test(sycl::queue& queue, oneapi::dpl::internal::element_type_t<IntType> left,
     oneapi::dpl::internal::element_type_t<IntType> right, int nsamples)
{

    // memory allocation
    std::vector<oneapi::dpl::internal::element_type_t<IntType>> samples(nsamples);

    constexpr std::int32_t num_elems = oneapi::dpl::internal::type_traits_t<IntType>::num_elems == 0
                                           ? 1
...
template <class IntType, class UIntType>
int
test_portion(sycl::queue& queue, oneapi::dpl::internal::element_type_t<IntType> left,
             oneapi::dpl::internal::element_type_t<IntType> right, int nsamples, unsigned int part)
{
