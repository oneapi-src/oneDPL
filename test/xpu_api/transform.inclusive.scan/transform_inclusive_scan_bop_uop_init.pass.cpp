  ...
  cl::sycl::range<1> numOfItems1{5};
  {
    ...
    deviceQueue.submit([&](cl::sycl::handler &cgh) {
      ...
      cgh.single_task<Iter>([=]() {
        oneapi::dpl::transform_inclusive_scan(Iter(&in[0]), Iter(&in[0] + 5),
                                              &out1[0], oneapi::dpl::plus<>(),
                                              add_one{}, 0);
        oneapi::dpl::transform_inclusive_scan(
            Iter(&in[0]), Iter(&in[0] + 5), &out2[0],
            oneapi::dpl::multiplies<>(), add_one{}, 0);
        oneapi::dpl::transform_inclusive_scan(Iter(&in[0]), Iter(&in[0] + 5),
                                              &out3[0], oneapi::dpl::plus<>(),
                                              oneapi::dpl::negate<>(), 0);
        oneapi::dpl::transform_inclusive_scan(
            Iter(&in[0]), Iter(&in[0] + 5), &out4[0],
            oneapi::dpl::multiplies<>(), oneapi::dpl::negate<>(), 0);
      });
    });
  }
