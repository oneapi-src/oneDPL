...
template <class T, class = decltype(dpl::tuple_size<T>::value)>
constexpr bool
has_value(int)
{
    return true;
...
kernel_test()
{
    ...
    deviceQueue.submit([&](sycl::handler& cgh) {
        ...
        cgh.single_task<class KernelTupleSizeValueSfinaeTest>([=]() {
            ret_access[0] = has_value<dpl::tuple<int> const>();
            ret_access[0] &= has_value<dpl::pair<int, long> volatile>();
            ret_access[0] &= !has_value<int>();
            ret_access[0] &= !has_value<const int>();
            ret_access[0] &= !has_value<volatile void>();
            ret_access[0] &= !has_value<const volatile dpl::tuple<int>&>();
        });
    });
