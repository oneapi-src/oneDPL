...
public:
    ...
    TestComplexFncReal(TErrorsContainer& ee)
        : errors(ee)
    {
    }
...
protected:
    ...
    {
#if __cplusplus >= 201402L
        TEST_KW_CONSTEXPR dpl::complex<T> cv(TestUtils::Complex::TestConstants<T>::kPartReal);
    ...
    {
#if __cplusplus >= 201402L
        TEST_KW_CONSTEXPR T z = TestUtils::Complex::TestConstants<T>::kPartReal;
        TEST_KW_CONSTEXPR auto real_res = dpl::real(z);
        static_assert(::std::is_same<typename TestUtils::Complex::TestConstants<T>::DestComplexFieldType, typename ::std::decay<decltype(real_res)>::type>::value);
...
main()
{
    bool bSuccess = TestUtils::Complex::test_on_host<TestComplexFncReal>();
    ...
    try
    {
        ...
        if (!TestUtils::Complex::test_in_kernel<TestComplexFncReal>(deviceQueue, TestUtils::kMaxKernelErrorsCount))
            bSuccess = false;
    }
