        ...
        deviceQueue.submit(
            [&](sycl::handler& cgh)
            {
                ...
                cgh.single_task<KernelTest<Iter>>(
                    [=]()
                    {
                        out[0] = dpl::reduce(Iter(&in[0]), Iter(&in[0]), 0, std::plus<>());
                        out[1] = dpl::reduce(Iter(&in[0]), Iter(&in[0]), 1, std::multiplies<>());
                        out[2] = dpl::reduce(Iter(&in[0]), Iter(&in[0] + 1), 0, std::plus<>());
                        out[3] = dpl::reduce(Iter(&in[0]), Iter(&in[0] + 1), 2, std::multiplies<>());
                        out[4] = dpl::reduce(Iter(&in[0]), Iter(&in[0] + 2), 0, std::plus<>());
                        out[5] = dpl::reduce(Iter(&in[0]), Iter(&in[0] + 2), 3, std::multiplies<>());
                        out[6] = dpl::reduce(Iter(&in[0]), Iter(&in[0] + 6), 0, std::plus<>());
                        out[7] = dpl::reduce(Iter(&in[0]), Iter(&in[0] + 6), 4, std::multiplies<>());
                    });
            });
    }
