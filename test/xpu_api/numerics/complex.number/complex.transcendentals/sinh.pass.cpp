...
void test_edges(TChecker& check_obj)
{
    ...
    for (unsigned i = 0; i < N; ++i)
    {
        ...
        {
#if !_PSTL_TEST_COMPLEX_SINH_BROKEN
            CALL_CHECK_OBJ_I(check_obj, i, r.real() == 0);
            CALL_CHECK_OBJ_I(check_obj, i, std::isnan(r.imag()));
#endif // _PSTL_TEST_COMPLEX_SINH_BROKEN
        }
        ...
        {
            CALL_CHECK_OBJ_I(check_obj, i, std::isinf(r.real()));
            CALL_CHECK_OBJ_I(check_obj, i, std::signbit(r.real()) == std::signbit(testcases[i].real() * std::cos(testcases[i].imag())));
            CALL_CHECK_OBJ_I(check_obj, i, std::isinf(r.imag()));
            CALL_CHECK_OBJ_I(check_obj, i, std::signbit(r.imag()) == std::signbit(dpl::sin(testcases[i].imag())));
        }
