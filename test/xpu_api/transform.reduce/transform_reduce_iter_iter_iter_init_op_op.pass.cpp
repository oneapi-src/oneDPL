...
template <class Iter1, class Iter2, class KernelTest> void test() {
    ...
    deviceQueue.submit([&](cl::sycl::handler &cgh) {
      ...
      cgh.single_task<KernelTest>([=]() {
        out[0] = oneapi::dpl::transform_reduce(
            Iter1(&in1[0]), Iter1(&in1[0]), Iter2(&in2[0]), 0,
            oneapi::dpl::plus<>(), oneapi::dpl::multiplies<>());
        out[1] = oneapi::dpl::transform_reduce(
            Iter2(&in2[0]), Iter2(&in2[0]), Iter1(&in1[0]), 1,
        ...
        out[6] = oneapi::dpl::transform_reduce(
            Iter1(&in1[0]), Iter1(&in1[0] + 6), Iter2(&in2[0]), 4,
            oneapi::dpl::multiplies<>(), oneapi::dpl::plus<>());
  ...
  cl::sycl::queue deviceQueue;
  {
    deviceQueue.submit([&](cl::sycl::handler &cgh) {
      cgh.single_task<KernelName>([=]() {
        T *p = nullptr;
        static_assert(
            oneapi::dpl::is_same_v<Init,
                                   decltype(oneapi::dpl::transform_reduce(
                                       p, p, p, Init{}, oneapi::dpl::plus<>(),
