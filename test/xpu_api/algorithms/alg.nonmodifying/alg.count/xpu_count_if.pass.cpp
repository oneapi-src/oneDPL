    ...
    sycl::range<1> item1{1};
    {
        ...
        deviceQueue.submit([&](sycl::handler& cgh) {
            ...
            cgh.single_task<class KernelTest1>([=]() {
                ...
                ret_acc[0] &=
                    (count_if(input_iterator<const int*>(ia), input_iterator<const int*>(ia + sa), eq(2)) == 3);
                ret_acc[0] &=
                    (count_if(input_iterator<const int*>(ia), input_iterator<const int*>(ia + sa), eq(7)) == 0);
                ret_acc[0] &= (count_if(input_iterator<const int*>(ia), input_iterator<const int*>(ia), eq(2)) == 0);
            });
        });
    }
    assert(ret);
