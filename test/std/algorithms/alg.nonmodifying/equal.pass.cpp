...
test(size_t bits)
{
    for (size_t n = 1; n <= 100000; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
    {
        ...
        invoke_on_all_policies(test_one_policy<T, 0>(), in.begin(), in.end(), inCopy.begin(), true);
        invoke_on_all_policies(test_one_policy<T, 1>(), in.cbegin(), in.cend(), inCopy.cbegin(), true);

        // testing bool !equal()
        inCopy[0] = !inCopy[0];
        invoke_on_all_policies(test_one_policy<T, 2>(), in.begin(), in.end(), inCopy.begin(), false);
        invoke_on_all_policies(test_one_policy<T, 3>(), in.cbegin(), in.cend(), inCopy.cbegin(), false);
    }
}
