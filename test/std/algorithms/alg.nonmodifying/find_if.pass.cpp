...
test(Predicate pred, Hit hit, Miss miss)
{
    ...
    for (size_t n = 0; n <= 100000; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
    {
        ...
        for (size_t m = n; m > 0; m *= 0.6)
        {
            if (m < n)
                in[m] = hit(n ^ m);
            invoke_on_all_policies(test_find_if<T, true>(), in.begin(), in.end(), pred, not_pred);
            invoke_on_all_policies(test_find_if<T, false>(), in.cbegin(), in.cend(), pred, not_pred);
            invoke_on_all_policies(test_find_if_not<T, true>(), in.begin(), in.end(), pred, not_pred);
            invoke_on_all_policies(test_find_if_not<T, false>(), in.cbegin(), in.cend(), pred, not_pred);
        }
    }
}
...
struct test_non_const_find_if
{
    ...
    operator()(Policy&& exec, Iterator iter)
    {
        ...
        invoke_if(exec, [&]() { find_if(exec, iter, iter, non_const(is_even)); });
    }
};
...
struct test_non_const_find_if_not
{
    ...
    operator()(Policy&& exec, Iterator iter)
    {
        ...
        invoke_if(exec, [&]() { find_if_not(exec, iter, iter, non_const(is_even)); });
    }
};
