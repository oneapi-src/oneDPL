...
template <typename Type, typename Predicate, bool Partition>
struct test_count
{
...
test(T needle, Predicate pred, Convert convert)
{
    ...
    for (size_t n = 0; n <= 100000; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
    {
        ...
        invoke_on_all_policies(test_count<In, Predicate, true>(), in.begin(), in.end(), needle);
        invoke_on_all_policies(test_count_if<In, Predicate, true>(), in.begin(), in.end(), pred);

        invoke_on_all_policies(test_count<In, Predicate, false>(), in.cbegin(), in.cend(), needle);
        invoke_on_all_policies(test_count_if<In, Predicate, false>(), in.cbegin(), in.cend(), pred);
    }
}
