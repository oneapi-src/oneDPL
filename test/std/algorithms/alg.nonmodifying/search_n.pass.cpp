...
template <typename Type, bool Partition>
    ...
    for (std::size_t n1 = 0; n1 <= max_n1; n1 = n1 <= 16 ? n1 + 1 : size_t(3.1415 * n1))
    {
        ...
        for (auto n2 : sub_n)
        {
            ...
            for (auto r : res)
            {
                ...
                invoke_on_all_policies(test_search_n<T, true>(), in.begin(), in.begin() + n1, n2, value,
                                       std::equal_to<T>());
                invoke_on_all_policies(test_search_n<T, false>(), in.cbegin(), in.cbegin() + n1, n2, value,
                                       std::equal_to<T>());

                invoke_on_all_policies(test_search_n_predicate<T, true>(), in.begin(), in.begin() + n1, n2, value,
                                       std::equal_to<T>());
                invoke_on_all_policies(test_search_n_predicate<T, false>(), in.cbegin(), in.cbegin() + n1, n2, value,
                                       std::equal_to<T>());
            }
        }
    }
