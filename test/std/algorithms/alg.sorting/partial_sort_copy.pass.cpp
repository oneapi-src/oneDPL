...
template <typename RandomAccessIterator, typename Type, std::size_t Partition>
...
test_partial_sort_copy(Compare compare)
{
    ...
    for (; n1 < n_max; n1 = n1 <= 16 ? n1 + 1 : size_t(3.1415 * n1))
    {
        ...
        invoke_on_all_policies(
            test_one_policy<iterator_type, T, 0>(out.begin(), out.begin() + n2, exp.begin(), exp.begin() + n2),
            in.begin(), in.begin() + n1, n1, n2, trash, compare);

        // If first sequence is greater than second
        n2 = n1 / 3;
        invoke_on_all_policies(
            test_one_policy<iterator_type, T, 1>(out.begin(), out.begin() + n2, exp.begin(), exp.begin() + n2),
            in.begin(), in.begin() + n1, n1, n2, trash, compare);

        // If first sequence is less than second
        n2 = 2 * n1;
        invoke_on_all_policies(
            test_one_policy<iterator_type, T, 2>(out.begin(), out.begin() + n2, exp.begin(), exp.begin() + n2),
            in.begin(), in.begin() + n1, n1, n2, trash, compare);
    }
    ...
    n2 = 2 * n1;
    invoke_on_all_policies(
        test_one_policy<iterator_type, T, 3>(out.begin(), out.begin() + n2, exp.begin(), exp.begin() + n2), in.begin(),
