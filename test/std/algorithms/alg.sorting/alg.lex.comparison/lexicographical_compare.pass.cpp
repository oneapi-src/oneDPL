...
test(Predicate pred)
{
    ...
    invoke_on_all_policies(test_one_policy<T1, 0>(), in1.cbegin(), in1.cbegin() + max_n, in2.cbegin() + 3 * max_n / 10,
    ...
    invoke_on_all_policies(test_one_policy<T1, 1>(), in1.begin(), in1.begin() + max_n, in2.cbegin(),
                           in2.cbegin() + max_n2, pred);
    invoke_on_all_policies(test_one_policy<T1, 2>(), in1.begin(), in1.begin() + max_n, in2.begin() + max_n2,
                           in2.begin() + 3 * max_n2, pred);

    // Test case: If one range is a prefix of another, the shorter range is lexicographically less than the other.
    max_n2 = 2 * max_n;
    invoke_on_all_policies(test_one_policy<T1, 3>(), in1.cbegin(), in1.cbegin() + max_n, in2.begin(),
    ...
    for (std::size_t n1 = 0; n1 <= max_n; n1 = n1 <= 16 ? n1 + 1 : std::size_t(3.1415 * n1))
    {
        ...
        n2 = n1;
        invoke_on_all_policies(test_one_policy<T1, 4>(), in1.begin(), in1.begin() + n1, in2.begin(), in2.begin() + n2,
                               pred);
        ...
        std::size_t ind = n1 / 2;
        in2[ind] = T2(-1);
        invoke_on_all_policies(test_one_policy<T1, 5>(), in1.begin(), in1.begin() + n1, in2.begin(), in2.begin() + n2,
                               pred);
        in2[ind] = T2(ind);
        ...
        ind = n1 / 5;
        in1[ind] = T1(-1);
        invoke_on_all_policies(test_one_policy<T1, 6>(), in1.begin(), in1.begin() + n1, in2.cbegin(), in2.cbegin() + n2,
                               pred);
        in1[ind] = T1(ind);
    }
}
...
test_string(Predicate pred)
{
    ...
    for (std::size_t n1 = 0; n1 < in1.size(); n1 = n1 <= 16 ? n1 + 1 : std::size_t(3.1415 * n1))
    {
        // Test case: If two ranges have equivalent elements and are of the same length, then the ranges are lexicographically equal.
        n2 = n1;
        invoke_on_all_policies(test_one_policy<Predicate, 0>(), in1.begin(), in1.begin() + n1, in2.begin(),
                               in2.begin() + n2, pred);

        n2 = n1;
        // Test case: two ranges have different elements and are of the same length (second sequence less than first)
        in2[n1 / 2] = 'a';
        invoke_on_all_policies(test_one_policy<Predicate, 1>(), in1.begin(), in1.begin() + n1, in2.begin(),
                               in2.begin() + n2, pred);

        // Test case: two ranges have different elements and are of the same length (first sequence less than second)
        in1[n1 / 5] = 'a';
        invoke_on_all_policies(test_one_policy<Predicate, 2>(), in1.begin(), in1.begin() + n1, in2.cbegin(),
                               in2.cbegin() + n2, pred);
    }
    invoke_on_all_policies(test_one_policy<Predicate, 3>(), in1.cbegin(), in1.cbegin() + max_n,
