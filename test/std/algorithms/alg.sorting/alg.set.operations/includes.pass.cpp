...
test_includes(Compare compare)
{
    ...
    for (std::size_t n = 0; n < n_max; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
    {
        for (std::size_t m = 0; m < n_max; m = m <= 16 ? m + 1 : size_t(2.71828 * m))
        {
            ...
            invoke_on_all_policies(test_one_policy<T1, true>(), in1.begin(), in1.end(), in2.cbegin(), in2.cend(),
                                   compare);

            //test w/ non constant predicate
            if (n < 5 && m < 5)
                invoke_on_all_policies(test_one_policy<T1, false>(), in1.begin(), in1.end(), in2.cbegin(), in2.cend(),
                                       non_const(compare));
        }
    }
}
...
main()
{
    ...
    test_includes<float64_t, float64_t>(pstl::__internal::__pstl_less());
