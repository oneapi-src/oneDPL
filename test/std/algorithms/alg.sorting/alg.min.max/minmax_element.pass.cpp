...
test_by_type(std::size_t n)
{
    ...
    for (std::set<std::size_t>::iterator it = targets.begin(); it != targets.end(); ++it)
    {
        ...
        TestUtils::invoke_on_all_policies(check_minelement<T, 0>(), wseq.seq.cbegin(), wseq.seq.cend());
        TestUtils::invoke_on_all_policies(check_minelement_predicate<T, 0>(), wseq.seq.cbegin(), wseq.seq.cend());
        TestUtils::invoke_on_all_policies(check_minelement<T, 1>(), wseq.seq.begin(), wseq.seq.end());
        TestUtils::invoke_on_all_policies(check_minelement_predicate<T, 1>(), wseq.seq.begin(), wseq.seq.end());

        wseq.set_desired_value(*it, wseq.max_value);
        TestUtils::invoke_on_all_policies(check_maxelement<T, 0>(), wseq.seq.cbegin(), wseq.seq.cend());
        TestUtils::invoke_on_all_policies(check_maxelement_predicate<T, 0>(), wseq.seq.cbegin(), wseq.seq.cend());
        TestUtils::invoke_on_all_policies(check_maxelement<T, 1>(), wseq.seq.begin(), wseq.seq.end());
        TestUtils::invoke_on_all_policies(check_maxelement_predicate<T, 1>(), wseq.seq.begin(), wseq.seq.end());
        ...
        if (targets.size() > 1)
        {
            for (std::set<std::size_t>::reverse_iterator rit = targets.rbegin(); rit != targets.rend(); ++rit)
            {
                ...
                TestUtils::invoke_on_all_policies(check_minmaxelement<T, 0>(), wseq.seq.cbegin(), wseq.seq.cend());
                TestUtils::invoke_on_all_policies(check_minmaxelement_predicate<T, 0>(), wseq.seq.cbegin(),
                                                  wseq.seq.cend());
                TestUtils::invoke_on_all_policies(check_minmaxelement<T, 1>(), wseq.seq.begin(), wseq.seq.end());
                TestUtils::invoke_on_all_policies(check_minmaxelement_predicate<T, 1>(), wseq.seq.begin(),
                                                  wseq.seq.end());
            }
        }
        else
        { // we must check this corner case; it can not be tested in loop above
            TestUtils::invoke_on_all_policies(check_minmaxelement<T, 2>(), wseq.seq.cbegin(), wseq.seq.cend());
            TestUtils::invoke_on_all_policies(check_minmaxelement_predicate<T, 2>(), wseq.seq.cbegin(),
                                              wseq.seq.cend());
            TestUtils::invoke_on_all_policies(check_minmaxelement<T, 3>(), wseq.seq.begin(), wseq.seq.end());
            TestUtils::invoke_on_all_policies(check_minmaxelement_predicate<T, 3>(), wseq.seq.begin(), wseq.seq.end());
        }
    }
}
