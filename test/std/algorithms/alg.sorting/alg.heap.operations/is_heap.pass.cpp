...
struct test_is_heap
{
    ...
    operator()(pstl::execution::unsequenced_policy, Iterator first, Iterator last)
    {
    }

    template <typename Iterator>
    typename std::enable_if<is_same_iterator_category<Iterator, std::random_access_iterator_tag>::value, void>::type
    operator()(pstl::execution::parallel_unsequenced_policy, Iterator first, Iterator last)
    {
    }
#endif
...
test_is_heap_by_type(Comp comp)
{
    ...
    for (size_t n = 0; n <= max_size; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
    {
        ...
        invoke_on_all_policies(test_is_heap<T, 0>(), in.begin(), in.end());
        invoke_on_all_policies(test_is_heap_predicate<T, 0>(), in.begin(), in.end(), comp);
        invoke_on_all_policies(test_is_heap_until<T, 0>(), in.begin(), in.end());
        invoke_on_all_policies(test_is_heap_until_predicate<T, 0>(), in.begin(), in.end(), comp);

        std::make_heap(in.begin(), in.begin() + n / 4, comp);
        invoke_on_all_policies(test_is_heap<T, 1>(), in.cbegin(), in.cend());
        invoke_on_all_policies(test_is_heap_predicate<T, 1>(), in.cbegin(), in.cend(), comp);
        invoke_on_all_policies(test_is_heap_until<T, 1>(), in.cbegin(), in.cend());
        invoke_on_all_policies(test_is_heap_until_predicate<T, 1>(), in.cbegin(), in.cend(), comp);

        std::make_heap(in.begin(), in.begin() + n / 3, comp);
        invoke_on_all_policies(test_is_heap<T, 2>(), in.begin(), in.end());
        invoke_on_all_policies(test_is_heap_predicate<T, 2>(), in.begin(), in.end(), comp);
        invoke_on_all_policies(test_is_heap_until<T, 2>(), in.begin(), in.end());
        invoke_on_all_policies(test_is_heap_until_predicate<T, 2>(), in.begin(), in.end(), comp);

        std::make_heap(in.begin(), in.end(), comp);
        invoke_on_all_policies(test_is_heap<T, 3>(), in.cbegin(), in.cend());
        invoke_on_all_policies(test_is_heap_predicate<T, 3>(), in.cbegin(), in.cend(), comp);
        invoke_on_all_policies(test_is_heap_until<T, 3>(), in.cbegin(), in.cend());
        invoke_on_all_policies(test_is_heap_until_predicate<T, 3>(), in.cbegin(), in.cend(), comp);
    }

    Sequence<T> in(max_size / 10, [](size_t v) -> T { return T(1); });
    invoke_on_all_policies(test_is_heap<T, 4>(), in.begin(), in.end());
    invoke_on_all_policies(test_is_heap_predicate<T, 4>(), in.begin(), in.end(), comp);
    invoke_on_all_policies(test_is_heap_until<T, 4>(), in.begin(), in.end());
    invoke_on_all_policies(test_is_heap_until_predicate<T, 4>(), in.begin(), in.end(), comp);
}
...
struct test_non_const_is_heap
{
    ...
    {
        invoke_if(exec, [&]() { is_heap(exec, iter, iter, non_const(std::less<T>())); });
    }
};
...
struct test_non_const_is_heap_until
{
    ...
    operator()(Policy&& exec, Iterator iter)
    {
        invoke_if(exec, [&]() { is_heap_until(exec, iter, iter, non_const(std::less<T>())); });
    }
};
