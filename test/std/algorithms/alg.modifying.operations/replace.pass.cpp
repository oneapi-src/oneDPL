...
template <typename Iterator, typename PartitionType>
struct test_replace
{
    std::size_t len;
    Iterator data_b;
    Iterator data_e;
    test_replace(Iterator data_, std::size_t len_)
    {
        len = len_;
        data_b = data_;
...
struct test_replace_if
{
    ...
    Iterator data_b;
    Iterator data_e;
    test_replace_if(Iterator data_, std::size_t len_)
    {
        len = len_;
        data_b = data_;
        data_e = std::next(data_b, len);
...
test(Pred pred)
{
    ...
    {
        test_replace<iterator_type, T1> temp1(data.begin(), len);
        test_replace_if<iterator_type, T1> temp2(data.begin(), len);

        invoke_on_all_policies(temp1, expected.begin(), expected.begin() + len, actual.begin(), actual.begin() + len,
                               pred, new_value, value);
        invoke_on_all_policies(temp2, expected.begin(), expected.begin() + len, actual.begin(), actual.begin() + len,
                               pred, new_value, value);
    }
}
...
main()
{
    test<int32_t, float32_t>(pstl::__internal::__equal_value<int32_t>(666));
