...
test(T trash, const T& old_value, const T& new_value, Predicate pred, Convert convert)
{
    ...
    for (size_t n = 0; n <= 100000; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
    {
        ...
        invoke_on_all_policies(test_replace_copy<T, true>(), in.begin(), in.end(), out.begin(), out.end(),
                               expected.begin(), expected.end(), out.size(), pred, old_value, new_value, trash);
        invoke_on_all_policies(test_replace_copy<T, false>(), in.cbegin(), in.cend(), out.begin(), out.end(),
                               expected.begin(), expected.end(), out.size(), pred, old_value, new_value, trash);
        invoke_on_all_policies(test_replace_copy_if<T, true>(), in.begin(), in.end(), out.begin(), out.end(),
                               expected.begin(), expected.end(), out.size(), pred, old_value, new_value, trash);
        invoke_on_all_policies(test_replace_copy_if<T, false>(), in.cbegin(), in.cend(), out.begin(), out.end(),
                               expected.begin(), expected.end(), out.size(), pred, old_value, new_value, trash);
    }
}
