...
test(Predicate pred)
{
    ...
    {
        invoke_on_all_policies(test_is_partitioned<T, true>(), in.begin(), in.begin() + n1, pred);
        std::partition(in.begin(), in.begin() + n1, pred);
        invoke_on_all_policies(test_is_partitioned<T, false>(), in.begin(), in.begin() + n1, pred);
    }
}
