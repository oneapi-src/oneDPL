...
template <typename T1, typename T2, typename T3, bool Partition>
...
test(Predicate pred)
{
    for (size_t n = 0; n <= 100000; n = n <= 16 ? n + 1 : size_t(3.1415 * n))
    {
        ...
        invoke_on_all_policies(test_one_policy<In1, In2, Out, true>(), in1.begin(), in1.end(), in2.begin(), in2.end(),
                               out.begin(), out.end(), pred);
        invoke_on_all_policies(test_one_policy<In1, In2, Out, false>(), in1.cbegin(), in1.cend(), in2.cbegin(),
...
main()
{
    ...
    // lambda
    test<int32_t, float64_t, int32_t>(
        [](const int32_t& x, const float64_t& y) { return int32_t(int32_t(1.5) + x - y); });
