...
test()
{
    ...
    for (std::size_t len = 0; len < max_len; len = len <= 16 ? len + 1 : std::size_t(3.1415 * len))
    {
        ...
        for (std::size_t shift : shifts)
        {
            ...
            {
                invoke_on_all_policies(test_one_policy<T1, true>(), data.begin(), data.begin() + len, actual.begin(),
                                       actual.begin() + len, shift);
                invoke_on_all_policies(test_one_policy<T1, false>(), data.cbegin(), data.cbegin() + len, actual.begin(),
                                       actual.begin() + len, shift);
            }
        }
    }
