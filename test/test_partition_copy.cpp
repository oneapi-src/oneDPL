...
struct test_partition_copy {
    ...
    void operator()(Policy&& exec, InputIterator first, InputIterator last, OutputIterator true_first, OutputIterator true_last,
        OutputIterator2 false_first, UnaryOp unary_op) {
    ...
    void operator()(pstl::execution::unsequenced_policy, std::reverse_iterator<InputIterator> first, std::reverse_iterator<InputIterator> last,
        ...
        std::reverse_iterator<OutputIterator2> false_first, UnaryOp unary_op) { }
    template <typename InputIterator, typename OutputIterator, typename OutputIterator2, typename UnaryOp>
...
void test( UnaryPred pred) {
    ...
    for (std::size_t n = 0; n <= 100000; n = n <= 16 ? n + 1 : std::size_t(3.1415 * n)) {
        Sequence<T> in(n, [](std::size_t v)->T { return T(v); });
        Sequence<T> actual_true(n);
        Sequence<T> actual_false(n);

        // for non-const input iterators
        invoke_on_all_policies(test_partition_copy(), in.begin(), in.end(),
            actual_true.begin(), actual_true.end(), actual_false.begin(), pred);

        // for const input iterators
        invoke_on_all_policies(test_partition_copy(), in.cbegin(), in.cend(),
            actual_true.begin(), actual_true.end(), actual_false.begin(), pred);
    }
}
