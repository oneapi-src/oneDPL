...
#    endif
    ...
    std::stable_sort(std::begin(output_ref), std::end(output_ref), Compare<T, IsAscending>{});
    {
        ...
        oneapi::dpl::experimental::kt::esimd::radix_sort_out_of_place<IsAscending>(q, view, view_out, param).wait();
    }
...
#    endif
    ...
    oneapi::dpl::experimental::kt::esimd::radix_sort_out_of_place<IsAscending>(q, view_in, view_out, param).wait();
...
test_usm(sycl::queue q, std::size_t size, KernelParam param)
{
    ...
    oneapi::dpl::experimental::kt::esimd::radix_sort_out_of_place<IsAscending>(
...
test_sycl_buffer(sycl::queue q, std::size_t size, KernelParam param)
{
    ...
    std::stable_sort(std::begin(output_ref), std::end(output_ref), Compare<T, IsAscending>{});
    {
        sycl::buffer<T> buf(input.data(), input.size());
        sycl::buffer<T> buf_out(output.data(), output.size());
        oneapi::dpl::experimental::kt::esimd::radix_sort_out_of_place<IsAscending>(q, buf, buf_out, param).wait();
    }
...
test_small_sizes(sycl::queue q, KernelParam param)
{
    ...
    oneapi::dpl::experimental::kt::esimd::radix_sort_out_of_place<IsAscending, RadixBits>(
