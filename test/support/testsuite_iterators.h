...
struct random_access_iterator_wrapper
  ...
  random_access_iterator_wrapper &operator+=(std::ptrdiff_t n) {
    if (n > 0) {
  ...
  random_access_iterator_wrapper operator-(std::ptrdiff_t n) const {
    random_access_iterator_wrapper<T> tmp = *this;
    return tmp -= n;
  }

  std::ptrdiff_t operator-(const random_access_iterator_wrapper<T> &in) const {
