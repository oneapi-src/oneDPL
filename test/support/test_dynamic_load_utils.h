...
test_submit_and_wait_on_group(UniverseContainer u, ResourceFunction&& f)
{
    ...
    // Create buffers for 2D arrays
    sycl::buffer<int, 2> bufferA(a.data(), sycl::range<2>(N, D));
    sycl::buffer<int, 2> bufferB(b.data(), sycl::range<2>(N, D));
    sycl::buffer<int, 2> bufferResultMatrix(resultMatrix.data(), sycl::range<2>(N, N));
                        ...
                        cgh.parallel_for<TestUtils::unique_kernel_name<class load2, 0>>(
                            sycl::range<1>(N), [=](sycl::item<1> item) {
                                for (size_t j = 0; j < N; ++j)
                                {
                                    ...
                                    accessorResultMatrix[item.get_id(0)][j] = dotProduct;
                                }
                            });
                    });
                    return e2;
    ...
    // Check if the expected number of resources were selected
