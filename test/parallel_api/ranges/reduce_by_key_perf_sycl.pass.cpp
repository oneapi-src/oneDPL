...
auto do_work_ranges(BufA A, BufB B, BufC C, BufD D, Res& res)
{
...
template<typename BufA, typename BufB, typename BufC, typename BufD, typename Res>
auto do_work_it(BufA A, BufB B, BufC C, BufD D, Res& res)
{
    using namespace oneapi::dpl::experimental::ranges;
...
main()
{
    ...
    auto res = 0; 
    auto time = (do_work_ranges(A, B, C, D, res), do_work_ranges(A, B, C, D, res)); //double call to init RT and warm the cache
    ::std::cout << "do_work ranges::reduce_by_key, time (ms): " << time << ::std::endl;
    ...
    auto res1 = 0; 
    auto time2 = (do_work_it(A, B, C1, D1, res1), do_work_it(A, B, C1, D1, res1)); //double call to init RT and warm the cache
    ::std::cout << "do_work iterators reduce_by_segment, time (ms): " << time2 << ::std::endl;
