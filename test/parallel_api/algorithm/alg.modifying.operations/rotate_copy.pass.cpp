...
struct test_one_policy
{
               ...
               ::std::size_t shift)
    {
        ...
        auto comparer = comparator<T2, Iterator1, Iterator2>();
...
test()
{
    ...
    for (::std::size_t len = 0; len < max_len; len = len <= 16 ? len + 1 : ::std::size_t(3.1415 * len))
    {
        ...
        for (::std::size_t shift : shifts)
        {
            ...
            {
                invoke_on_all_policies<0>()(test_one_policy<T1, T2>(), data.begin(), data.begin() + len, actual.begin(),
                                            actual.begin() + len, shift);
                invoke_on_all_policies<1>()(test_one_policy<T1, T2>(), data.cbegin(), data.cbegin() + len, actual.begin(),
                                            actual.begin() + len, shift);
            }
        }
    }
