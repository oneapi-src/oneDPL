...
main()
{
    ...
    test_set<test_set_union, data_t, data_t>(oneapi::dpl::__internal::__pstl_less(), false);
#if !ONEDPL_FPGA_DEVICE
    test_set<test_set_union, data_t, data_t>(oneapi::dpl::__internal::__pstl_less(), true);
#endif

#if !TEST_DPCPP_BACKEND_PRESENT
    test_set<test_set_union, Num<std::int64_t>, Num<std::int32_t>>(
        [](const Num<std::int64_t>& x, const Num<std::int32_t>& y) { return x < y; }, true);

    test_set<test_set_union, MemoryChecker, MemoryChecker>(
