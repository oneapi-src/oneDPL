...
main()
{
    ...
    test_set<test_set_difference, data_t, data_t>(oneapi::dpl::__internal::__pstl_less(), false);
#if !ONEDPL_FPGA_DEVICE
    test_set<test_set_difference, data_t, data_t>(oneapi::dpl::__internal::__pstl_less(), true);
#endif

#if !TEST_DPCPP_BACKEND_PRESENT
    test_set<test_set_difference, Num<std::int64_t>, Num<std::int32_t>>(
        [](const Num<std::int64_t>& x, const Num<std::int32_t>& y) { return x < y; }, true);

    test_set<test_set_difference, MemoryChecker, MemoryChecker>(
...
#endif // _PSTL_TEST_SET_DIFFERENCE
