...
int main()
{
    ...
    test_sort<TestUtils::float32_t>(cfg.msg("float, host, custom less"), sizes, Host{},
                                    Converter<TestUtils::float32_t>{}, ConstLess{});
    test_sort<std::int16_t>(cfg.msg("int16_t, host, non-const custom less"), sizes, Host{},
                            Converter<std::int16_t>{}, NonConstLess{});
    test_sort<std::uint32_t>(cfg.msg("uint32_t, host, std::greater"), sizes, Host{},
    ...
    test_sort<TestUtils::float32_t>(cfg.msg("float, device, custom less"), sizes, Device<0>{},
                                    Converter<TestUtils::float32_t>{}, ConstLess{});
    ...
    test_sort<std::uint16_t>(cfg.msg("uint16_t, device, ConstGreater"), extended_sizes, Device<1>{},
                             Converter<std::uint16_t>{}, ConstGreater{});
#if __SYCL_UNNAMED_LAMBDA__
    // Test potentially clashing naming for radix sort descending / ascending with minimal timing impact
    test_default_name_gen(cfg.msg("default name generation"));
#endif
