...
template<typename _Policy, typename _KeyIt, typename _ValIt, typename _Size, typename StabilityTag>
void
call_sort_by_key(_Policy&& policy, _KeyIt keys_begin, _ValIt vals_begin, _Size n, StabilityTag)
{
...
template<typename _Policy, typename StabilityTag>
void
test_with_std_policy(_Policy&& policy, StabilityTag stability_tag)
{
...
template <sycl::usm::alloc alloc_type, typename StabilityTag>
void
test_with_usm(sycl::queue& q, StabilityTag)
{
    constexpr int N = 32;
...
void
test_with_buffers(sycl::queue& q, StabilityTag stability_tag)
{
    constexpr int n = 1000000;
    ...
    // call algo
    call_sort_by_key(policy, keys_begin, vals_begin, n, stability_tag);
...
test_all_policies(StabilityTag stability_tag)
{
    ...
    test_with_usm<sycl::usm::alloc::shared>(q, stability_tag);
    // Run tests for USM device memory
    test_with_usm<sycl::usm::alloc::device>(q, stability_tag);
    // Run tests for sycl buffers
    test_with_buffers(q, stability_tag);
#endif // TEST_DPCPP_BACKEND_PRESENT

#if !TEST_DPCPP_BACKEND_PRESENT
    test_with_std_policy(oneapi::dpl::execution::seq, stability_tag);
    test_with_std_policy(oneapi::dpl::execution::unseq, stability_tag);
    test_with_std_policy(oneapi::dpl::execution::par, stability_tag);
