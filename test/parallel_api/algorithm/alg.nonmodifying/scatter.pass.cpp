...
void
scatter(Policy&& policy, InputIter1 first, InputIter1 last, InputIter2 map, OutputIter result)
{
    oneapi::dpl::copy(policy, first, last, oneapi::dpl::make_permutation_iterator(result, map));
}

void
test_scatter_with_usm(int input_size)
{
    sycl::queue q;
...
void
test_scatter_with_buffers(int input_size)
{
    ...
    // call algorithm
    scatter(oneapi::dpl::execution::dpcpp_default, src_beg, src_end, map_beg, res_beg);
...
main()
{
    ...
    {
        test_scatter_with_usm(n);
        test_scatter_with_buffers(n);
    }
