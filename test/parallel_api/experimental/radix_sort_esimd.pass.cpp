...
void test_all_view(std::size_t size)
{
    ...
    std::stable_sort(std::begin(ref), std::end(ref), Compare<T, Order>{});
    {
        ...
        oneapi::dpl::experimental::kt::esimd::radix_sort<WorkGroupSize, DataPerWorkItem, Order>(policy, view);
    }
...
void test_usm(std::size_t size)
{
    ...
    oneapi::dpl::experimental::kt::esimd::radix_sort<WorkGroupSize, DataPerWorkItem, Order>(policy, dt_input.get_data(), dt_input.get_data() + size);
...
void test_sycl_iterators(std::size_t size)
{
    ...
    {
        sycl::buffer<T> buf(input.data(), input.size());
        oneapi::dpl::experimental::kt::esimd::radix_sort<WorkGroupSize, DataPerWorkItem, Order>(policy, oneapi::dpl::begin(buf), oneapi::dpl::end(buf));
    }
...
void test_small_sizes()
{
    ...
    oneapi::dpl::experimental::kt::esimd::radix_sort<WorkGroupSize, DataPerWorkItem, Ascending>(policy, oneapi::dpl::begin(input), oneapi::dpl::begin(input));
    EXPECT_EQ_RANGES(ref, input, "sort modified input data when size == 0");
    oneapi::dpl::experimental::kt::esimd::radix_sort<WorkGroupSize, DataPerWorkItem, Ascending>(policy, oneapi::dpl::begin(input), oneapi::dpl::begin(input) + 1);
