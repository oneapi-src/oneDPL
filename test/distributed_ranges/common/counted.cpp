...
TYPED_TEST(Counted, Basic) {
  ...
  auto dist = xp::views::counted(ops.dist_vec.begin() + 1, 2);
...
TYPED_TEST(Counted, Mutate) {
  ...
  EXPECT_TRUE(
      check_mutate_view(ops, stdrng::views::counted(ops.vec.begin() + 2, 3),
                        xp::views::counted(ops.dist_vec.begin() + 2, 3)));
}

template <class TypeParam>
void localAndDrCountedResultsAreSameTest(std::size_t countedSize) {
  Ops1<TypeParam> ops(10);
  auto dist = xp::views::counted(ops.dist_vec.begin() + 2, countedSize);
...
TYPED_TEST(Counted, emptyInput_zeroSize) {
  TypeParam dv(0);
  auto dist = xp::views::counted(dv.begin(), 0);
...
TYPED_TEST(Counted, large) {
  ...
  auto counted_result = xp::views::counted(dv.begin() + 2, 54321);
...
TYPED_TEST(Counted, countedOfOneElementHasOneSegmentAndSameRank) {
  TypeParam dv(10, 77);
  auto counted_view_result = xp::views::counted(dv.end() - 1, 1);
...
TYPED_TEST(Counted, countedOfFirstSegementHasOneSegmentAndSameRank) {
  ...
  auto counted_view_result =
      xp::views::counted(dv.begin() + bias, first_seg_size - bias);
  auto counted_view_segments = dr::ranges::segments(counted_view_result);
  EXPECT_EQ(stdrng::size(counted_view_segments), 1);
  EXPECT_EQ(dr::ranges::rank(counted_view_segments[0]),
...
TYPED_TEST(Counted, countedOfAllButOneSizeHasAllSegmentsWithSameRanks) {
  ...
  auto counted_view_result =
      xp::views::counted(dv.begin() + bias, EVENLY_DIVIDABLE_SIZE - bias);
  auto counted_view_segments = dr::ranges::segments(counted_view_result);
