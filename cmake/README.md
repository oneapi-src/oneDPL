...
| ONEDPL_EXPLICIT_KERNEL_NAMES | STRING | Control usage of Kernel names in tests [AUTO, ALWAYS]: if AUTO then Kenel names are not used in tests when unnamed lambda option is switched on; if ALWAYS then Kernel names are used in tests explicitly | AUTO          |
