...
struct sequence_wrapper {
    ...
    void pattern_fill() {
        seq.fill( [](std::size_t i) -> T { return T(HashBits(i, bits)); });
    }
    ...
    void set_desired_value(std::size_t at, T value) {
        ...
        for(std::size_t i = at + 1; i < seq.size(); i += 1 + HashBits(i, 5))
            seq[i] = value;
    }
};
...
void test_by_type(std::size_t n){
    ...
    for(std::set<std::size_t>::iterator it = targets.begin(); it != targets.end(); ++it) {
        ...
        invoke_on_all_policies(check_minelement(), wseq.seq.cbegin(), wseq.seq.cend());
        invoke_on_all_policies(check_minelement(), wseq.seq.begin(), wseq.seq.end());

        wseq.set_desired_value(*it, wseq.max_value);
        invoke_on_all_policies(check_maxelement(), wseq.seq.cbegin(), wseq.seq.cend());
        invoke_on_all_policies(check_maxelement(), wseq.seq.begin(), wseq.seq.end());
        ...
        if(targets.size() > 1) {
            for(std::set<std::size_t>::reverse_iterator rit = targets.rbegin(); rit != targets.rend(); ++rit) {
                ...
                invoke_on_all_policies(check_minmaxelement(), wseq.seq.cbegin(), wseq.seq.cend());
                invoke_on_all_policies(check_minmaxelement(), wseq.seq.begin(), wseq.seq.end());
            }
...
int32_t main() {
    using Parallel_TestUtils::float64_t;
