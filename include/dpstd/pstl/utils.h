...
__pstl_upper_bound(_Acc __acc, _Size1 __first, _Size1 __last, const _Value& __value, _Compare __comp)
{
    return __pstl_lower_bound(__acc, __first, __last, __value,
                              dpstd::__internal::__not_pred<dpstd::__internal::__reorder_pred<_Compare>>{
...
{
    auto __beg = _ReverseCounter<_Index>{__last - 1};
    auto __end = _ReverseCounter<_Index>{__first - 1};
...
template <std::size_t _Np>
using __make_index_sequence = typename dpstd::__internal::__make_index_sequence_impl<_Np>::type;
