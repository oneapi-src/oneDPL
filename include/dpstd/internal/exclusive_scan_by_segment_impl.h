...
namespace internal {
    ...
    exclusive_scan_by_segment_impl(Policy&& policy, InputIterator1 first1, InputIterator1 last1,
                                   ...
                                   BinaryPredicate binary_pred, Operator binary_op)
    {
        ...
        // compute head flags
        dpstd::__par_backend::__buffer<policy_type, FlagType> _flags(n);
        ...
        dpstd::__par_backend::__buffer<policy_type, OutputType> _temp(n);
        auto temp = _temp.get();
...
#else
         replace_copy_if(policy1, first2, last2 - 1, _flags.get() + 1, _temp.get() + 1,
        ...
        inclusive_scan(policy2,
                       make_zip_iterator(_temp.get(), _flags.get()),
                       make_zip_iterator(_temp.get(), _flags.get()) + n,
                       make_zip_iterator(result, _flags.get()),
                                   ...
                                   BinaryPredicate binary_pred, Operator binary_op)
    {
        ...
        // scan key-flag tuples
        transform_inclusive_scan(policy2,
                       make_zip_iterator(_temp.get(), _flags.get()),
                       make_zip_iterator(_temp.get(), _flags.get()) + n,
                       make_zip_iterator(result, _flags.get()),
                       internal::segmented_scan_fun<ValueType, FlagType, Operator>(binary_op),
                       dpstd::__internal::__no_op(), dpstd::__par_backend_hetero::__internal::make_tuple(init, FlagType(1))
        );
    ...
    exclusive_scan_by_segment(Policy&& policy, InputIterator1 first1, InputIterator1 last1,
                              ...
                              BinaryPredicate binary_pred, Operator binary_op)
    {
        return internal::exclusive_scan_by_segment_impl(std::forward<Policy>(policy), first1, last1,
