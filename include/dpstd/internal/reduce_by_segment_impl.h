...
namespace internal {
    ...
    template<typename Policy, typename InputIterator1, typename InputIterator2,
             typename OutputIterator1, typename OutputIterator2, typename BinaryPred,
             typename BinaryOperator>
    dpstd::__internal::__enable_if_host_execution_policy<typename std::decay<Policy>::type, std::pair<OutputIterator1,OutputIterator2>>
    reduce_by_segment_impl(Policy&& policy, InputIterator1 first1, InputIterator1 last1,
                           InputIterator2 first2, OutputIterator1 result1, OutputIterator2 result2,
                           BinaryPred binary_pred, BinaryOperator binary_op)
    {
        ...
        dpstd::__par_backend::__buffer<policy_type, FlagType> _mask(n + 1);
        auto mask = _mask.get();
        mask[0] = 1;
        ...
        dpstd::__par_backend::__buffer<policy_type, FlagType> _scanned_values(n);
        ...
        dpstd::__par_backend::__buffer<policy_type, FlagType> _scanned_tail_flags(n);
        ...
        inclusive_scan(policy1,
                       make_zip_iterator(first2, _mask.get()),
        ...
        std::exclusive_scan(policy2, _mask.get() + 1, _mask.get() + n + 1,
                            _scanned_tail_flags.get(), CountType(0), std::plus<CountType>());
    ...
    template<typename Policy, typename InputIterator1, typename InputIterator2,
             typename OutputIterator1, typename OutputIterator2, typename BinaryPred,
             typename BinaryOperator>
    dpstd::__internal::__enable_if_hetero_execution_policy<typename std::decay<Policy>::type, std::pair<OutputIterator1,OutputIterator2>>
    reduce_by_segment_impl(Policy&& policy, InputIterator1 first1, InputIterator1 last1,
                  InputIterator2 first2, OutputIterator1 result1, OutputIterator2 result2,
                  BinaryPred binary_pred, BinaryOperator binary_op)
    {
        ...
        // Compute the sum of the segments. scanned_tail_flags values are not used.
        typename internal::rebind_policy<policy_type, class ReduceByKey1>::type policy1(policy);
        transform_inclusive_scan(policy1,
                       make_zip_iterator(first2, _mask.get()),
                       make_zip_iterator(first2, _mask.get()) + n,
                       make_zip_iterator(_scanned_values.get(), _scanned_tail_flags.get()),
                       internal::segmented_scan_fun<ValueType, FlagType, BinaryOperator>(binary_op),
                       dpstd::__internal::__no_op(), std::make_tuple(initial_value, initial_mask)
        );
        ...
        {
            auto permute_it1 = internal::make_permutation_iterator(result1, _scanned_tail_flags.get());
            std::for_each(policy3,
                          make_zip_iterator(first1, _mask.get() + 1, permute_it1),
                          make_zip_iterator(last1, _mask.get() + 1 + n, permute_it1 + n),
                          internal::transform_if_stencil_fun<FlagType,identity>(identity()));
        }

        {
            auto permute_it2 = internal::make_permutation_iterator(result2, _scanned_tail_flags.get());
            std::for_each(policy4,
                          make_zip_iterator(_scanned_values.get(), _mask.get() + 1, permute_it2),
    ...
    {
        return internal::reduce_by_segment_impl(std::forward<Policy>(policy), first1, last1, first2,
                                            result1, result2, binary_pred, binary_op);
    }

    template<typename Policy, typename InputIt1, typename InputIt2, typename OutputIt1,
             typename OutputIt2, typename BinaryPred>
    dpstd::__internal::__enable_if_execution_policy<Policy, std::pair<OutputIt1,OutputIt2>>
    reduce_by_segment(Policy&& policy, InputIt1 first1, InputIt1 last1, InputIt2 first2,
                  OutputIt1 result1, OutputIt2 result2, BinaryPred binary_pred)
    {
    ...
    template<typename Policy, typename InputIt1, typename InputIt2, typename OutputIt1,
             typename OutputIt2, typename BinaryPred>
    dpstd::__internal::__enable_if_execution_policy<Policy, std::pair<OutputIt1,OutputIt2>>
    reduce_by_key(Policy&& policy, InputIt1 first1, InputIt1 last1, InputIt2 first2,
                  OutputIt1 result1, OutputIt2 result2, BinaryPred binary_pred)
    {
        return reduce_by_segment(std::forward<Policy>(policy), first1, last1, first2, result1, result2,
                                 binary_pred);
