...
struct sycl_scan_by_segment_impl
{
    ...
    void
    sycl_inclusive_scan_by_segment(_ExecutionPolicy&& __exec, _Range1&& __keys, _Range2&& __values,
                                   _Range3&& __out_values, _BinaryPredicate __binary_pred, _BinaryOperator __binary_op)
    {
        ...
        auto __end_idx =
            oneapi::dpl::__par_backend_hetero::__internal::__buffer<_ExecutionPolicy, __val_type>(__exec, 1)
                .get_buffer();
        ...
        // 1. Work group scan
        auto __wg_incl_scan = __exec.queue().submit([&](sycl::handler& __cgh) {
            ...
            __cgh.parallel_for(
                sycl::nd_range<1>{__n_groups * __wgroup_size, __wgroup_size}, [=](sycl::nd_item<1> __item) {
                    ...
                    // 1d. Store partials and presence of segment flags for use in phase 2
                    if (__local_id == __wgroup_size - 1) // last work item writes the group's carry out
                    {
                        ...
                        if (__max_end == 0)
                            __partials_acc[__group_id] =
                                __binary_op(__carry_in, __accumulator); // inclusive with last element

                        else
                            __partials_acc[__group_id] = __accumulator;

                        bool __seg_present = static_cast<bool>(__closest_seg_id);
            ...
            .submit([&](sycl::handler& __cgh) {
                ...
                __cgh.parallel_for(
                    sycl::nd_range<1>{__n_groups * __wgroup_size, __wgroup_size}, [=](sycl::nd_item<1> __item) {
                        ...
                        // 2a.Check to see if an aggregate exists and compute 
                        // that value in the first work item.
                        ...
                        // find the smallest segment end index in the work item
                        for (int32_t __i = __start; __i < __end; ++__i)
                        {
                            if (__i == __n - 1 || __keys[__i] != __keys[__i + 1])
                            {
                        ...
                        ::std::size_t __wg_min_seg_end =
                            sycl::reduce_over_group(__group, __local_min_key_idx, sycl::minimum<>());
    ...
    void
    sycl_exclusive_scan_by_segment(_ExecutionPolicy&& __exec, _Range1&& __keys, _Range2&& __values,
                                   _Range3&& __out_values, _BinaryPredicate __binary_pred, _BinaryOperator __binary_op,
                                   _T __init = _T{})
    {
        using __diff_type = oneapi::dpl::__internal::__difference_t<_Range1>;
        ...
        auto __wg_excl_scan = __exec.queue().submit([&](sycl::handler& __cgh) {
            auto __partials_acc = __partials.template get_access<sycl::access_mode::write>(__cgh);
            auto __seg_ends_acc = __seg_ends.template get_access<sycl::access_mode::write>(__cgh);
            ...
            __cgh.parallel_for(
                sycl::nd_range<1>{__n_groups * __wgroup_size, __wgroup_size}, [=](sycl::nd_item<1> __item) {
                    ...
                    if (__local_id == __wgroup_size - 1) // last work item writes the group's carry out
                    {
                        ...
                        __flag_type __seg_present = static_cast<__flag_type>(__closest_seg_id);
                        __seg_ends_acc[__group_id] = __seg_present;
                    }
                });
        });
    ...
    void operator()(_ExecutionPolicy&& __exec, _Range1&& __keys, _Range2&& __values, _Range3&& __out_values,
                    _BinaryPredicate __binary_pred, _BinaryOperator __binary_op, T init = T{})
    {
        if constexpr (__scan_type == scan_type::exclusive)
        {
            sycl_exclusive_scan_by_segment(::std::forward<_ExecutionPolicy>(__exec), ::std::forward<_Range1>(__keys),
                                           ...
                                           __binary_pred, __binary_op, init);
        }
        else
        {
