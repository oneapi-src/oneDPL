...
template<class _CharT, class _Traits, typename __UIntType, ::std::size_t __w, ::std::size_t __n, ::std::size_t __r, __UIntType...__consts>
...
public:
    ...
    static_assert(_w > 0 && _w <= ::std::numeric_limits<scalar_type>::digits, "_w must satisfy 0 < _w < ::std::numeric_limits<_UIntType>::digits");
    static_assert(::std::numeric_limits<scalar_type>::digits <= 64, "size of the scalar _UIntType (in case of sycl::vec<T, N> the size of T) must be less than 64 bits");
    ...
    void seed(scalar_type __seed = default_seed) { seed_internal({ __seed & in_mask }); }

    /* Set the state to arbitrary position */
    void set_counter(const ::std::array<scalar_type, word_count>& __counter) {
    ...
    /* Equality operators */
    friend bool operator==(const philox_engine& __x, const philox_engine& __y) {
...
private:
    ...
    void increase_counter_internal(unsigned long long __z) {
    ...
    generate_internal(unsigned int __random_nums) {
        ...
        for (int __elm_count = 0; __elm_count < __random_nums; __elm_count++) {
            if(__curr_idx  == word_count) { // empty buffer
                philox_kernel();
    ...
    void discard_internal(unsigned long long __z) {
...
template<class _CharT, class _Traits, typename __UIntType, ::std::size_t __w, ::std::size_t __n, ::std::size_t __r, __UIntType...__consts>
::std::basic_ostream<_CharT, _Traits>& 
operator<<(::std::basic_ostream<_CharT, _Traits>& __os, const philox_engine<__UIntType, __w, __n, __r, __consts...>& __engine) {
