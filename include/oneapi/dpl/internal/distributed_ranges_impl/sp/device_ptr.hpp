...
requires(std::is_trivially_copyable_v<T> || std::is_void_v<T>) class device_ptr
{
  public:
    ...
    bool
    operator==(const device_ptr&) const noexcept = default;
    bool
    operator!=(const device_ptr&) const noexcept = default;
    ...
    difference_type
    operator-(pointer other) const noexcept
    {
        return pointer_ - other.pointer_;
    }
    ...
    T*
    get_raw_pointer() const noexcept
    {
        return pointer_;
    }

    friend pointer
    operator+(difference_type n, pointer iter)
    {
        return iter + n;
    }
