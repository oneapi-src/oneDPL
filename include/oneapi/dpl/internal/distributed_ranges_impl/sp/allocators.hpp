...
requires(std::is_trivially_copyable_v<T>) class device_allocator
{
  public:
    ...
    device_allocator(const sycl::queue& q) noexcept : device_(q.get_device()), context_(q.get_context()) {}
    device_allocator(const sycl::context& ctxt, const sycl::device& dev) noexcept : device_(dev), context_(ctxt) {}
    ...
    allocate(std::size_t size)
    {
        ...
        {
            return pointer(sycl::aligned_alloc_device<T>(Alignment, size, device_, context_));
        }
    }
    ...
    bool
    operator==(const device_allocator&) const = default;
    bool
    operator!=(const device_allocator&) const = default;
...
class buffered_allocator
{
  public:
    ...
    buffered_allocator(const Allocator& alloc, std::size_t buffer_size, std::size_t n_buffers)
        : alloc_(alloc), buffer_size_(buffer_size), free_buffers_(new std::vector<pointer>()),
    ...
    bool
    operator==(const buffered_allocator&) const = default;
    bool
    operator!=(const buffered_allocator&) const = default;
