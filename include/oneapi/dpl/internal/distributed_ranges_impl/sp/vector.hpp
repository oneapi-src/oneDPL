...
class vector
{
  public:
    ...
    explicit vector(size_type count, const T& value, const Allocator& alloc = Allocator()) : allocator_(alloc)
    {
    ...
    explicit vector(size_type count, const Allocator& alloc = Allocator()) : allocator_(alloc)
    {
        change_capacity_impl_(count);
        using namespace std;
    ...
    constexpr vector(Iter first, Iter last, const Allocator& alloc = Allocator()) : allocator_(alloc)
    {
        change_capacity_impl_(stdrng::distance(first, last));
        using namespace std;
    ...
    {
        auto new_size = stdrng::distance(first, last);
        reserve(new_size);
        using namespace std;
        copy(first, last, begin());
        size_ = new_size;
  ...
  private:
    ...
    void
    change_capacity_impl_(size_type count)
    {
    ...
    //       (Public domain)
    constexpr size_type
    next_highest_power_of_two_impl_(size_type n)
    {
        n--;
        n |= n >> 1;
        n |= n >> 2;
