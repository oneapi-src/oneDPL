...
class zip_accessor
{
  ...
  private:
    ...
    reference
    get_impl_(std::index_sequence<Ints...>) const noexcept
    {
...
class zip_view : public stdrng::view_interface<zip_view<Rs...>>
{
  public:
    ...
    segments() const requires(distributed_range<Rs> || ...)
    {
        ...
        using segment_view_type =
            decltype(get_zipped_view_impl_(segment_ids, local_idx, 0, std::make_index_sequence<sizeof...(Rs)>{}));
        ...
        while (cumulative_size < size())
        {
            ...
            // (e.g. get_view<0>.rank())
            auto segment_view =
                get_zipped_view_impl_(segment_ids, local_idx, size, std::make_index_sequence<sizeof...(Rs)>{});
    ...
    zipped_segments() const requires(distributed_range<Rs> || ...)
    {
        ...
        using segment_view_type =
            decltype(get_zipped_segments_impl_(segment_ids, local_idx, 0, std::make_index_sequence<sizeof...(Rs)>{}));
        std::vector<segment_view_type> segment_views;
        ...
        while (cumulative_size < size())
        {
            ...
            auto segment_view =
                get_zipped_segments_impl_(segment_ids, local_idx, size, std::make_index_sequence<sizeof...(Rs)>{});
  ...
  private:
    ...
    {
        return stdrng::views::zip(ranges::local_or_identity(std::get<Ints>(views_))...);
    }
    ...
    auto
    get_zipped_view_impl_(auto&& segment_ids, auto&& local_idx, std::size_t size, std::index_sequence<Is...>) const
    {
        return zip_view<decltype(create_view_impl_(segment_or_orig_(get_view<Is>(), segment_ids[Is]))
                                     .subspan(local_idx[Is], size))...>(
            create_view_impl_(segment_or_orig_(get_view<Is>(), segment_ids[Is])).subspan(local_idx[Is], size)...);
    }

    template <std::size_t... Is>
    auto
    get_zipped_segments_impl_(auto&& segment_ids, auto&& local_idx, std::size_t size, std::index_sequence<Is...>) const
    {
        return std::tuple(
            create_view_impl_(segment_or_orig_(get_view<Is>(), segment_ids[Is])).subspan(local_idx[Is], size)...);
    }
    ...
    void
    increment_local_idx(auto&& segment_ids, auto&& local_idx, std::size_t size) const
    {
        local_idx[I] += size;
    ...
    decltype(auto)
    segment_or_orig_(T&& t, std::size_t idx) const
    {
        return ranges::segments(t)[idx];
    }
    ...
    std::size_t
    get_next_segment_size(auto&& segment_ids, auto&& local_idx) const
    {
