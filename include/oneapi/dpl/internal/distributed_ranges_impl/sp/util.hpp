...
sycl::device
select_device(Selector&& selector)
{
    sycl::device d;
...
inline void
print_device_details(std::span<sycl::device> devices)
{
    std::size_t device_id = 0;
    for (auto&& device : devices)
    {
        std::cout << "Device " << device_id << ": " << device.get_info<sycl::info::device::name>() << std::endl;
        device_id++;
    }
}
...
template <typename Selector>
void
list_devices(Selector&& selector)
{
    ...
    for (std::size_t i = 0; i < devices.size(); i++)
    {
        ...
        auto subdevices = device.create_sub_devices<sycl::info::partition_property::partition_by_affinity_domain>(
...
get_numa_devices_impl_(Selector&& selector)
{
    ...
    {
        auto subdevices = root_device.create_sub_devices<sycl::info::partition_property::partition_by_affinity_domain>(
            sycl::info::partition_affinity_domain::numa);
...
template <typename Selector>
std::vector<sycl::device>
get_numa_devices(Selector&& selector)
{
    try
    {
    ...
    catch (sycl::exception const& e)
    {
        ...
        {
            std::cerr << "NUMA partitioning not supported, returning root devices..." << std::endl;
...
template <typename Selector>
std::vector<sycl::device>
get_duplicated_devices(Selector&& selector, std::size_t n)
{
...
void
print_range(Range&& r, std::string label = "")
{
    std::size_t indent = 1;
...
template <typename R>
void
print_range_details(R&& r, std::string label = "")
{
    if (label != "")
    {
        std::cout << "\"" << label << "\" ";
    ...
    {
        std::cout << "Seg " << idx++ << ", size " << segment.size() << " (rank " << ranges::rank(segment) << ")"
                  << std::endl;
    }
}
