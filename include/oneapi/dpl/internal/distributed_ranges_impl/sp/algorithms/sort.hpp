...
sort(R&& r, Compare comp = Compare())
{
    ...
    using dpl_future_type = decltype(__detail::sort_async(
        __detail::dpl_policy(0), stdrng::begin(ranges::__detail::local_or_identity(*stdrng::begin(segments))),
        stdrng::end(ranges::__detail::local_or_identity(*stdrng::begin(segments))), comp));
    ...
    for (auto&& [segment_id, segment] : dr::__detail::make_enumerate(segments))
    {
        ...
        // local_segment]
        std::size_t* splitter_i = sycl::malloc_shared<std::size_t>(n_segments, q.get_device(), sp::context());
        ...
        T* medians_l = sycl::malloc_device<T>(n_splitters, q.get_device(), sp::context());
        ...
        sycl::free(medians_l, sp::context());
    ...
    for (auto&& [segment_id_, segment] : dr::__detail::make_enumerate(segments))
    {
        ...
        for (std::size_t i = 0; i < n_segments; i++)
        {
            ...
            auto e = sp::copy_async(p_first, p_last, sorted_segments[i] + pos);
            events.push_back(e);
...
#pragma omp parallel num_threads(n_segments)
    ...
    for (std::size_t i = 0; i < sorted_segments.size(); i++)
    {
        ...
        auto e = sp::copy_async(seg, seg + n_elements, d_first);
    ...
    {
        sycl::free(sorted_seg, sp::context());
    }

    for (auto&& splitter_i : splitter_indices)
    {
        sycl::free(splitter_i, sp::context());
    }

    sycl::free(medians, sp::context());
}
