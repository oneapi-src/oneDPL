...
class device_span : public experimental::dr::shp::span<T, Iter> {
public:
  constexpr device_span() noexcept {}
  ...
  template <rng::random_access_range R>
    requires(experimental::dr::remote_range<R>)
  device_span(R &&r)
      : experimental::dr::shp::span<T, Iter>(rng::begin(r), rng::size(r)),
        rank_(experimental::dr::ranges::rank(r)) {}

  template <rng::random_access_range R>
  device_span(R &&r, std::size_t rank)
      : experimental::dr::shp::span<T, Iter>(rng::begin(r), rng::size(r)), rank_(rank) {}

  template <class It>
  constexpr device_span(It first, std::size_t count, std::size_t rank)
      : experimental::dr::shp::span<T, Iter>(first, count), rank_(rank) {}

  template <class It, class End>
  constexpr device_span(It first, End last, std::size_t rank)
      : experimental::dr::shp::span<T, Iter>(first, last), rank_(rank) {}
