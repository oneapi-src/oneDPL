...
public:
  ...
  remote_span(R &&r)
      : span<T, Iter>(rng::begin(r), rng::size(r)), rank_(ranges::rank(r)) {}

  template <rng::random_access_range R>
  remote_span(R &&r, std::size_t rank)
      : span<T, Iter>(rng::begin(r), rng::size(r)), rank_(rank) {}

  template <class It>
  constexpr remote_span(It first, std::size_t count, std::size_t rank)
      : span<T, Iter>(first, count), rank_(rank) {}

  template <class It, class End>
  constexpr remote_span(It first, End last, std::size_t rank)
      : span<T, Iter>(first, last), rank_(rank) {}

  constexpr std::size_t rank() const noexcept { return rank_; }

  remote_span first(std::size_t n) const {
    return remote_span(this->begin(), this->begin() + n, rank_);
  }

  remote_span last(std::size_t n) const {
    return remote_span(this->end() - n, this->end(), rank_);
  }

  remote_span subspan(std::size_t offset, std::size_t count) const {
    return remote_span(this->begin() + offset, this->begin() + offset + count,
                       rank_);
  }
...
template <rng::random_access_range R>
remote_span(R &&, std::size_t)
    -> remote_span<rng::range_value_t<R>, rng::iterator_t<R>>;
