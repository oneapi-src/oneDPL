...
template <typename T>
sycl::usm::alloc
get_pointer_type(device_ptr<T> ptr)
{
...
template <typename T>
sycl::device
get_pointer_device(device_ptr<T> ptr)
{
...
sycl::queue&
get_queue_for_pointer(InputIt iter)
{
...
sycl::queue&
get_queue_for_pointers(InputIt iter, OutputIt iter2)
{
    if (get_pointer_type(iter) == sycl::usm::alloc::device)
    {
        auto device = get_pointer_device(iter);
...
inline sycl::event
combine_events(sycl::queue& q, const std::vector<sycl::event>& events)
{
...
inline sycl::event
combine_events(const std::vector<sycl::event>& events)
{
...
inline void
wait(const std::vector<sycl::event>& events)
{
