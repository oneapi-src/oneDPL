...
class distributed_vector_accessor
{
  public:
    ...
    constexpr distributed_vector_accessor(std::span<segment_type> segments, size_type segment_id, size_type idx,
                                          size_type segment_size) noexcept
        : segments_(segments), segment_id_(segment_id), idx_(idx), segment_size_(segment_size)
    {
    }
...
template <typename T, typename L>
using distributed_vector_iterator = iterator_adaptor<distributed_vector_accessor<T, L>>;
...
struct distributed_vector
{
  public:
    ...
    distributed_vector(std::size_t count = 0)
    {
        ...
        size_ = count;
        segment_size_ = (count + devices().size() - 1) / devices().size();
        capacity_ = segment_size_ * devices().size();

        std::size_t rank = 0;
        for (auto&& device : devices())
        {
            segments_.emplace_back(segment_type(segment_size_, Allocator(context(), device), rank++));
        }
    }

    distributed_vector(std::size_t count, const T& value) : distributed_vector(count) { fill(*this, value); }
    ...
    begin()
    {
        return iterator(segments_, 0, 0, segment_size_);
    }
    ...
    {
        return size_ ? iterator(segments_, size() / segment_size_, size() % segment_size_, segment_size_) : begin();
    }

    const_iterator
    end() const
    {
        return size_ ? const_iterator(segments_, size() / segment_size_, size() % segment_size_, segment_size_)
                     : begin();
    }
