...
OutputIt lower_bound(LocalPolicy &&policy, InputIt1 start, InputIt1 end,
  ...
  experimental::dr::__detail::direct_iterator d_start(start);
  experimental::dr::__detail::direct_iterator d_end(end);

  experimental::dr::__detail::direct_iterator d_value_first(value_first);
  experimental::dr::__detail::direct_iterator d_value_last(value_last);

  experimental::dr::__detail::direct_iterator d_result(result);
...
template <experimental::dr::distributed_range R, typename Compare = std::less<>>
void sort(R &&r, Compare comp = Compare()) {
  ...
  if (rng::size(segments) == 0) {
    ...
    auto &&local_policy =
        experimental::dr::shp::__detail::dpl_policy(experimental::dr::ranges::rank(segment));
    auto &&local_segment = experimental::dr::shp::__detail::local(segment);
  ...
  for (auto &&segment : segments) {
    auto &&q = experimental::dr::shp::__detail::queue(experimental::dr::ranges::rank(segment));
    auto &&local_policy =
        experimental::dr::shp::__detail::dpl_policy(experimental::dr::ranges::rank(segment));
  ...
  experimental::dr::shp::__detail::wait(events);
  events.clear();

  // Compute global medians by sorting medians and
  // computing `n_splitters` medians from the medians.
  auto &&local_policy = experimental::dr::shp::__detail::dpl_policy(0);
  ...
  auto &&q = experimental::dr::shp::__detail::queue(0);
  q.single_task([=] {
  ...
  for (std::size_t i = 0; i < sorted_segments.size(); i++) {
    auto &&local_policy =
        experimental::dr::shp::__detail::dpl_policy(experimental::dr::ranges::rank(segments[i]));
...
template <experimental::dr::distributed_iterator RandomIt, typename Compare = std::less<>>
