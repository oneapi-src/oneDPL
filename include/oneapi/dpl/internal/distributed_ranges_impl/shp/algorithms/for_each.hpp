...
template <typename ExecutionPolicy, oneapi::dpl::experimental::dr::distributed_range R, typename Fn>
void for_each(ExecutionPolicy &&policy, R &&r, Fn &&fn) {
  ...
  for (auto &&segment : oneapi::dpl::experimental::dr::ranges::segments(r)) {
    auto &&q = __detail::queue(oneapi::dpl::experimental::dr::ranges::rank(segment));
    ...
    auto event = oneapi::dpl::experimental::dr::__detail::parallel_for(
...
template <typename ExecutionPolicy, oneapi::dpl::experimental::dr::distributed_iterator Iter, typename Fn>
...
template <oneapi::dpl::experimental::dr::distributed_range R, typename Fn> void for_each(R &&r, Fn &&fn) {
  for_each(oneapi::dpl::experimental::dr::shp::par_unseq, std::forward<R>(r), std::forward<Fn>(fn));
}

template <oneapi::dpl::experimental::dr::distributed_iterator Iter, typename Fn>
void for_each(Iter begin, Iter end, Fn &&fn) {
  for_each(oneapi::dpl::experimental::dr::shp::par_unseq, begin, end, std::forward<Fn>(fn));
}
