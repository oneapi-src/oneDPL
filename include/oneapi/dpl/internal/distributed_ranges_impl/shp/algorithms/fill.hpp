...
sycl::event fill_async(Iter first, Iter last,
  ...
  return oneapi::dpl::experimental::dr::__detail::parallel_for(q, sycl::range<>(last - first),
...
template <typename T, oneapi::dpl::experimental::dr::remote_contiguous_range R>
sycl::event fill_async(R &&r, const T &value) {
  auto &&q = __detail::queue(oneapi::dpl::experimental::dr::ranges::rank(r));
  auto *arr = std::to_address(rng::begin(oneapi::dpl::experimental::dr::ranges::local(r)));
  // not using q.fill because of CMPLRLLVM-46438
  return oneapi::dpl::experimental::dr::__detail::parallel_for(q, sycl::range<>(rng::distance(r)),
                                    [=](auto idx) { arr[idx] = value; });
}
...
template <typename T, oneapi::dpl::experimental::dr::distributed_contiguous_range DR>
sycl::event fill_async(DR &&r, const T &value) {
  std::vector<sycl::event> events;

  for (auto &&segment : oneapi::dpl::experimental::dr::ranges::segments(r)) {
    auto e = oneapi::dpl::experimental::dr::shp::fill_async(segment, value);
    events.push_back(e);
  }
...
template <typename T, oneapi::dpl::experimental::dr::distributed_iterator Iter>
