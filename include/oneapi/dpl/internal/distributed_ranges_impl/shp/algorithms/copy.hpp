...
sycl::event copy_async(InputIt first, InputIt last, OutputIt d_first) {
  // auto &&q = experimental::dr::shp::__detail::default_queue();
...
template <std::forward_iterator InputIt, experimental::dr::distributed_iterator OutputIt>
  requires __detail::is_syclmemcopyable<std::iter_value_t<InputIt>,
                                        std::iter_value_t<OutputIt>>
sycl::event copy_async(InputIt first, InputIt last, OutputIt d_first) {
  auto &&segments = experimental::dr::ranges::segments(d_first);
  ...
  while (first != last) {
    ...
    events.emplace_back(
        experimental::dr::shp::copy_async(first, local_last, rng::begin(segment)));
...
auto copy(experimental::dr::distributed_range auto r, std::contiguous_iterator auto d_first) {
  return copy(rng::begin(r), rng::end(r), d_first);
}
...
template <experimental::dr::distributed_iterator InputIt, std::forward_iterator OutputIt>
  requires __detail::is_syclmemcopyable<std::iter_value_t<InputIt>,
                                        std::iter_value_t<OutputIt>>
sycl::event copy_async(InputIt first, InputIt last, OutputIt d_first) {
  auto dist = rng::distance(first, last);
  auto segments =
      experimental::dr::__detail::take_segments(experimental::dr::ranges::segments(first), dist);
  ...
  for (auto &&segment : segments) {
    ...
    events.emplace_back(
        experimental::dr::shp::copy_async(rng::begin(segment), rng::end(segment), d_first));
...
template <experimental::dr::distributed_iterator InputIt, experimental::dr::distributed_iterator OutputIt>
  requires __detail::is_syclmemcopyable<std::iter_value_t<InputIt>,
                                        std::iter_value_t<OutputIt>>
sycl::event copy_async(InputIt first, InputIt last, OutputIt d_first) {
  auto dist = rng::distance(first, last);
...
template <experimental::dr::distributed_range R, experimental::dr::distributed_iterator O>
