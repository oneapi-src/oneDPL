...
template <std::forward_iterator InputIt, oneapi::dpl::experimental::dr::distributed_iterator OutputIt>
  requires __detail::is_syclmemcopyable<std::iter_value_t<InputIt>,
                                        std::iter_value_t<OutputIt>>
sycl::event copy_async(InputIt first, InputIt last, OutputIt d_first) {
  auto &&segments = oneapi::dpl::experimental::dr::ranges::segments(d_first);
  ...
  while (first != last) {
    ...
    events.emplace_back(
        oneapi::dpl::experimental::dr::shp::copy_async(first, local_last, rng::begin(segment)));

    ++segment_iter;
    rng::advance(first, n_to_copy);
  }

  return oneapi::dpl::experimental::dr::shp::__detail::combine_events(events);
}

auto copy(rng::contiguous_range auto r, oneapi::dpl::experimental::dr::distributed_iterator auto d_first) {
  return copy(rng::begin(r), rng::end(r), d_first);
}

auto copy(oneapi::dpl::experimental::dr::distributed_range auto r, std::contiguous_iterator auto d_first) {
  return copy(rng::begin(r), rng::end(r), d_first);
}
...
template <oneapi::dpl::experimental::dr::distributed_iterator InputIt, std::forward_iterator OutputIt>
  requires __detail::is_syclmemcopyable<std::iter_value_t<InputIt>,
                                        std::iter_value_t<OutputIt>>
sycl::event copy_async(InputIt first, InputIt last, OutputIt d_first) {
  auto dist = rng::distance(first, last);
  auto segments =
      oneapi::dpl::experimental::dr::__detail::take_segments(oneapi::dpl::experimental::dr::ranges::segments(first), dist);
  ...
  for (auto &&segment : segments) {
    ...
    events.emplace_back(
        oneapi::dpl::experimental::dr::shp::copy_async(rng::begin(segment), rng::end(segment), d_first));
...
template <oneapi::dpl::experimental::dr::distributed_iterator InputIt, oneapi::dpl::experimental::dr::distributed_iterator OutputIt>
  requires __detail::is_syclmemcopyable<std::iter_value_t<InputIt>,
                                        std::iter_value_t<OutputIt>>
sycl::event copy_async(InputIt first, InputIt last, OutputIt d_first) {
  auto dist = rng::distance(first, last);
...
template <oneapi::dpl::experimental::dr::distributed_range R, oneapi::dpl::experimental::dr::distributed_iterator O>
