...
inline std::size_t
max_kernel_size_(std::size_t block_size = 128)
{
...
sycl::event
parallel_for_workaround(sycl::queue& q, sycl::range<1> numWorkItems, Fn&& fn, std::size_t block_size = 128)
{
    std::size_t num_blocks = (numWorkItems.size() + block_size - 1) / block_size;

    int32_t range_size = numWorkItems.size();

    auto event = q.parallel_for(sycl::nd_range<>(num_blocks * block_size, block_size), [=](auto nd_idx) {
...
template <typename Fn>
sycl::event
parallel_for_64bit(sycl::queue& q, sycl::range<1> numWorkItems, Fn&& fn)
{
    std::size_t block_size = 128;
    std::size_t max_kernel_size = max_kernel_size_(block_size);

    std::vector<sycl::event> events;
    for (std::size_t base_idx = 0; base_idx < numWorkItems.size(); base_idx += max_kernel_size)
    {
        std::size_t launch_size = std::min(numWorkItems.size() - base_idx, max_kernel_size);
...
//
inline auto
partitionable(sycl::device device)
{
    ...
    {
        if (e.code() == sycl::errc::invalid || e.code() == sycl::errc::feature_not_supported)
        {
            return false;
        }
        else
...
template <int Dim>
auto
nd_range(sycl::range<Dim> global)
{
    if constexpr (Dim == 1)
    {
        sycl::range local(128);
        return sycl::nd_range<Dim>(sycl::range(round_up(global[0], local[0])), local);
    }
    else if constexpr (Dim == 2)
    {
        sycl::range local(16, 16);
        return sycl::nd_range<Dim>(sycl::range(round_up(global[0], local[0]), round_up(global[1], local[1])), local);
    }
    else if constexpr (Dim == 3)
    {
        sycl::range local(8, 8, 8);
        return sycl::nd_range<Dim>(
            sycl::range(round_up(global[0], local[0]), round_up(global[1], local[1]), round_up(global[2], local[2])),
            local);
    }
    else
    {
...
template <typename Fn>
sycl::event
parallel_for_nd(sycl::queue& q, sycl::range<1> global, Fn&& fn)
{
...
template <typename Fn>
sycl::event
parallel_for_nd(sycl::queue& q, sycl::range<2> global, Fn&& fn)
{
    return q.parallel_for(nd_range(global), [=](auto nd_idx) {
        auto idx0 = nd_idx.get_global_id(0);
...
template <typename Fn>
sycl::event
parallel_for_nd(sycl::queue& q, sycl::range<3> global, Fn&& fn)
{
    return q.parallel_for(nd_range(global), [=](auto nd_idx) {
        auto idx0 = nd_idx.get_global_id(0);
        auto idx1 = nd_idx.get_global_id(1);
...
auto
combine_events(sycl::queue& q, const auto& events)
{
...
template <typename Fn>
sycl::event
parallel_for(sycl::queue& q, sycl::range<1> numWorkItems, Fn&& fn)
{
    std::size_t block_size = 128;
    ...
    {
        return parallel_for_workaround(q, numWorkItems, std::forward<Fn>(fn), block_size);
    }
    else
    {
...
template <typename Fn>
sycl::event
parallel_for(sycl::queue& q, sycl::range<2> global, Fn&& fn)
{
...
template <typename Fn>
sycl::event
parallel_for(sycl::queue& q, sycl::range<3> global, Fn&& fn)
{
    auto max = std::numeric_limits<std::int32_t>::max();
