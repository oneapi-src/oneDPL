...
struct custom_brick
{
    ...
    void
    operator()(_ItemId idx, _Acc1 acc)
    {
        T start_orig = 0;
        auto end_orig = size;
        using ::std::get;
                 ...
                 InputIterator2 value_end, OutputIterator result, StrictWeakOrdering comp)
{
    ...
    __parallel_for(::std::forward<Policy>(policy),
                   custom_brick<StrictWeakOrdering, decltype(size), lower_bound>{comp, size}, value_size, zip_vw);
                 ...
                 InputIterator2 value_end, OutputIterator result, StrictWeakOrdering comp)
{
    ...
    auto zip_vw = make_zip_view(input_buf.all_view(), value_buf.all_view(), result_buf.all_view());
    __parallel_for(::std::forward<Policy>(policy),
                   custom_brick<StrictWeakOrdering, decltype(size), upper_bound>{comp, size}, value_size, zip_vw);
    return result + value_size;
}
                   ...
                   InputIterator2 value_end, OutputIterator result, StrictWeakOrdering comp)
{
    ...
    auto zip_vw = make_zip_view(input_buf.all_view(), value_buf.all_view(), result_buf.all_view());
    __parallel_for(::std::forward<Policy>(policy),
                   custom_brick<StrictWeakOrdering, decltype(size), binary_search>{comp, size}, value_size, zip_vw);
    return result + value_size;
}
