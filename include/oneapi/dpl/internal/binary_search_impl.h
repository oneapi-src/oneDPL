  ...
  template<typename Comp, typename T, int func>
	...
	void operator()(_ItemId idx, _Acc1 acc)
	    ...
	    switch(func)
	      {
		  case 0: 
		      get<2>(acc[idx]) = oneapi::dpl::__internal::__pstl_lower_bound(get<0>(acc.tuple()),
                          start_orig, end_orig, get<1>(acc[idx]), comp);
		      break;
	          case 1:
		      get<2>(acc[idx]) = oneapi::dpl::__internal::__pstl_upper_bound(get<0>(acc.tuple()),
                          start_orig, end_orig, get<1>(acc[idx]), comp);
		      break;
	          case 2:
		      auto value = oneapi::dpl::__internal::__pstl_lower_bound(get<0>(acc.tuple()),
                          start_orig, end_orig, get<1>(acc[idx]), comp);
    ...
    lower_bound_impl(Policy&& policy, InputIterator1 start, InputIterator1 end,
		     InputIterator2 value_start, InputIterator2 value_end, OutputIterator result, StrictWeakOrdering comp)
    {
        using namespace __par_backend_hetero::__ranges;
        const auto size = std::distance(start, end);
	const auto value_size = std::distance(value_start, value_end);

        auto keep_input =
            oneapi::dpl::__ranges::__get_sycl_range<__par_backend_hetero::access_mode::read, InputIterator1>();
        auto input_buf = keep_input(start, end);
    
        auto keep_values =
            oneapi::dpl::__ranges::__get_sycl_range<__par_backend_hetero::access_mode::read, InputIterator2>();
        auto value_buf = keep_values(value_start, value_end);

        auto keep_result =
            oneapi::dpl::__ranges::__get_sycl_range<__par_backend_hetero::access_mode::read_write, OutputIterator>();
	...
	__parallel_for(std::forward<Policy>(policy), custom_brick<StrictWeakOrdering, decltype(size), lower_bound>{comp, size}, value_size, zip_vw);
	...
	const auto value_size = std::distance(value_start, value_end);
        ...
        auto result_buf = keep_result(result, result + value_size);
        auto zip_vw = make_zip_view(input_buf.all_view(), value_buf.all_view(), result_buf.all_view());
        __parallel_for(std::forward<Policy>(policy), custom_brick<StrictWeakOrdering, decltype(size), upper_bound>{comp, size}, value_size, zip_vw);
	return result + value_size;
    }
                     ...
                     InputIterator2 value_start, InputIterator2 value_end, OutputIterator result, StrictWeakOrdering comp)
    {
        ...
        auto result_buf = keep_result(result, result + value_size);
        auto zip_vw = make_zip_view(input_buf.all_view(), value_buf.all_view(), result_buf.all_view());
	__parallel_for(std::forward<Policy>(policy), custom_brick<StrictWeakOrdering, decltype(size), binary_search>{comp, size}, value_size, zip_vw);
	return result + value_size;
    }
...
namespace dpstd
{
using oneapi::dpl::binary_search;
using oneapi::dpl::upper_bound;
using oneapi::dpl::lower_bound;
} // end namespace dpstd
#endif
