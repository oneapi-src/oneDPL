...
void
__one_wg_kernel(sycl::nd_item<1> __idx, ::std::uint32_t __n, const InputT& __input)
{
    ...
    constexpr ::std::uint32_t _REORDER_SLM_SIZE = _DataPerWorkItem * sizeof(_KeyT) * _WorkGroupSize;
    constexpr ::std::uint32_t _BIN_HIST_SLM_SIZE = _HIST_STRIDE * _WorkGroupSize;
    constexpr ::std::uint32_t _INCOMING_OFFSET_SLM_SIZE = (_BIN_COUNT + 1) * sizeof(_hist_t);
    ...
    const ::std::uint32_t __slm_reorder_this_thread = __local_tid * _DataPerWorkItem * sizeof(_KeyT);
...
#pragma unroll
    ...
    for (::std::uint32_t __stage = 0; __stage < _STAGES; __stage++)
    {
        ...
        __bin_offset = 0;
#pragma unroll
        for (::std::uint32_t __s = 0; __s < _DataPerWorkItem; __s += 1)
        {
        ...
        */
        {
            barrier();
#pragma unroll
            for (::std::uint32_t __s = 0; __s < _BIN_COUNT; __s += 128)
            {
                __utils::_BlockStoreSlm<::std::uint32_t, 64>(
            ...
            if (__local_tid < _BIN_SUMMARY_GROUP_SIZE)
            {
                ...
                constexpr ::std::uint32_t _BIN_WIDTH_UD = _BIN_WIDTH * sizeof(_hist_t) / sizeof(::std::uint32_t);
                ...
                ___thread_grf_hist_summary.template bit_cast_view<::std::uint32_t>() =
                    __utils::_BlockLoadSlm<::std::uint32_t, _BIN_WIDTH_UD>(__slm_bin_hist_summary_offset);
                __slm_bin_hist_summary_offset += _HIST_STRIDE;
                for (::std::uint32_t __s = 1; __s < _WorkGroupSize - 1; __s++)
                {
                    __tmp = __utils::_BlockLoadSlm<::std::uint32_t, _BIN_WIDTH_UD>(__slm_bin_hist_summary_offset);
                    ___thread_grf_hist_summary += __tmp.template bit_cast_view<_hist_t>();
                    __utils::_BlockStoreSlm<::std::uint32_t, _BIN_WIDTH_UD>(
...
#pragma unroll
                for (::std::uint32_t __s = 0; __s < _BIN_COUNT; __s += 128)
                {
                    __grf_hist_summary.template select<128, 1>(__s).template bit_cast_view<::std::uint32_t>() =
                        __utils::_BlockLoadSlm<::std::uint32_t, 64>((_WorkGroupSize - 1) * _HIST_STRIDE + __s * sizeof(_hist_t));
                }
                __grf_hist_summary_scan[0] = 0;
                __grf_hist_summary_scan.template select<32, 1>(1) = __grf_hist_summary.template select<32, 1>(0);
#pragma unroll
                for (::std::uint32_t __i = 32; __i < _BIN_COUNT; __i += 32)
                {
...
#pragma unroll
                ...
                {
                    __bin_offset.template select<128, 1>(__s).template bit_cast_view<::std::uint32_t>() =
                        __utils::_BlockLoadSlm<::std::uint32_t, 64>(_BIN_HIST_SLM_SIZE + __s * sizeof(_hist_t));
                }
                if (__local_tid > 0)
                {
#pragma unroll
                    for (::std::uint32_t __s = 0; __s < _BIN_COUNT; __s += 128)
                    {
                        ...
                        __group_local_sum.template bit_cast_view<::std::uint32_t>() =
                            __utils::_BlockLoadSlm<::std::uint32_t, 64>((__local_tid - 1) * _HIST_STRIDE + __s * sizeof(_hist_t));
...
#pragma unroll
        for (::std::uint32_t __s = 0; __s < _DataPerWorkItem; __s += _DATA_PER_STEP)
        {
            simd<::std::uint16_t, _DATA_PER_STEP> __bins_uw = __bins.template select<_DATA_PER_STEP, 1>(__s);
