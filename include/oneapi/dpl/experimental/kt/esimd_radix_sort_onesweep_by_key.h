...
void global_histogram_by_key(sycl::nd_item<1> idx, size_t __n, const InputT& input, uint32_t *p_global_hist) {
    ...
    lsc_atomic_update<atomic_op::add>(p_global_hist + local_id * HIST_DATA_PER_WORK_ITEM, byte_offsets, group_hist,
                                      simd_mask<HIST_DATA_PER_WORK_ITEM>(1));
}

template <uint32_t BITS>
inline __ESIMD_NS::simd<uint32_t, 32> match_bins_by_key(__ESIMD_NS::simd<uint32_t, 32> bins, uint32_t local_tid) {
...
template <typename T>
struct slm_lookup_t_by_key {
    uint32_t slm;
    inline slm_lookup_t_by_key(uint32_t slm): slm(slm) {}
...
struct radix_sort_onesweep_slm_reorder_kernel_by_key {
    ...
    template <uint32_t CHUNK_SIZE, typename T, typename InputT>
    inline void Load(uint32_t io_offset, __ESIMD_NS::simd<T, PROCESS_SIZE> &load_to, const InputT& load_from, T default_value) const {
        using namespace __ESIMD_NS;
        using namespace __ESIMD_ENS;
        ...
        if (is_full_block) {
            ...
            for (uint32_t s = 0; s<PROCESS_SIZE; s+=CHUNK_SIZE) {
                sycl::ext::intel::esimd::simd offset((io_offset + s + lane_id) * sizeof(T));
                load_to.template select<CHUNK_SIZE, 1>(s) = lsc_gather<T, 1, lsc_data_size::default_size, cache_hint::cached, cache_hint::cached, 16>(load_from, offset);
            }
        }
        else
        {
            ...
            for (uint32_t s = 0; s<PROCESS_SIZE; s+=CHUNK_SIZE) {
                ...
                sycl::ext::intel::esimd::simd offset((io_offset + s + lane_id) * sizeof(T));
                load_to.template select<CHUNK_SIZE, 1>(s) =
                    merge(
                        lsc_gather<T, 1, lsc_data_size::default_size, cache_hint::cached, cache_hint::cached, 16>(load_from, offset, m),
                        simd<T, CHUNK_SIZE>(default_value),
                        m);
            }
        }
    }
    ...
    void operator() (sycl::nd_item<1> idx) const SYCL_ESIMD_KERNEL {
        ...
        Load<16>(io_offset, keys, keys_input, default_key);
        Load<16>(io_offset, values, values_input, ValueT{});
...
class __esimd_radix_sort_onesweep_histogram_by_key;

template <typename... _Name>
class __esimd_radix_sort_onesweep_scan_by_key;
...
struct __radix_sort_onesweep_histogram_submitter_by_key;

template <typename KeyT, ::std::uint32_t RADIX_BITS, ::std::uint32_t STAGES, ::std::uint32_t HW_TG_COUNT,
          ::std::uint32_t THREAD_PER_TG, bool IsAscending, typename... _Name>
struct __radix_sort_onesweep_histogram_submitter_by_key<KeyT, RADIX_BITS, STAGES, HW_TG_COUNT, THREAD_PER_TG, IsAscending,
...
struct __radix_sort_onesweep_scan_submitter_by_key;

template <::std::uint32_t STAGES, ::std::uint32_t BINCOUNT, typename... _Name>
struct __radix_sort_onesweep_scan_submitter_by_key<STAGES, BINCOUNT,
                                            oneapi::dpl::__par_backend_hetero::__internal::__optional_kernel_name<_Name...>>
{
...
onesweep_by_key(sycl::queue __q, KeysRange&& __keys, ValuesRange __values, ::std::size_t __n)
{
    ...
    using _EsimdRadixSortHistogram = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
            __esimd_radix_sort_onesweep_histogram_by_key<_KernelName>>;
    using _EsimdRadixSortScan = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
            __esimd_radix_sort_onesweep_scan_by_key<_KernelName>>;
    ...
    event_chain = __radix_sort_onesweep_histogram_submitter_by_key<
    ...
    if constexpr (STAGES % 2 != 0)
    {
        event_chain = __radix_sort_copyback_submitter_by_key<KeyT, _EsimdRadixSortCopyback>()(
            __q, __keys_tmp, __keys, __n, event_chain);
        event_chain = __radix_sort_copyback_submitter_by_key<ValueT, _EsimdRadixSortCopyback>()(
            __q, __values_tmp, __values, __n, event_chain);
    }
    event_chain.wait();
