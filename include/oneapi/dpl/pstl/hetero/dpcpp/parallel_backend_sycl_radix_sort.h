...
template <typename _T>
struct __ordered<
    _T, __enable_if_t<(std::is_integral<_T>::value && std::is_signed<_T>::value) || std::is_floating_point<_T>::value>>
{
...
template <typename _T>
inline __enable_if_t<!std::is_same<_T, __ordered_t<_T>>::value && !std::is_floating_point<_T>::value, __ordered_t<_T>>
__convert_to_ordered(_T __value)
{
...
template <typename _Iterator, std::uint32_t __radix_bits, bool __is_comp_asc, typename _ExecutionPolicy,
          typename _ValBuf, typename _CountBuf>
sycl::event
__radix_sort_count_submit(_ExecutionPolicy&& __exec, std::size_t __wgroups, std::size_t __block_size,
                          ...
                          _CountBuf& __count_buf, std::size_t __count_buf_size, sycl::event __dependency_event)
{
    ...
    using _SyclPolicy = __decay_t<_ExecutionPolicy>;
    using _KernelName = __radix_sort_count_kernel<_ValBuf, _CountBuf, typename _SyclPolicy::kernel_name>;
    ...
    const std::size_t __blocks_per_wgroup = __get_roundedup_div(__blocks_total, __wgroups);
    ...
    sycl::event __count_levent = __exec.queue().submit([&](sycl::handler& __hdl) {
        ...
        __hdl.parallel_for<_KernelName>(
            sycl::nd_range<1>(__wgroups * __block_size, __block_size), [=](sycl::nd_item<1> __self_item) {
                // item info
                const std::size_t __self_lidx = __self_item.get_local_id(0);
                const std::size_t __wgroup_idx = __self_item.get_group(0);
                const std::size_t __start_idx = __blocks_per_wgroup * __block_size * __wgroup_idx + __self_lidx;
                ...
                for (std::size_t __block_idx = 0; __block_idx < __blocks_per_wgroup; ++__block_idx)
                {
                    ...
                    // get bit values in a certain bucket of a value
                    std::uint32_t __bucket_val = __get_bucket_value<__radix_bits, __is_comp_asc>(__val, __radix_iter);
                ...
                for (std::uint32_t __active_ths = __block_size >> 1; __active_ths >= __radix_states; __active_ths >>= 1)
                {
...
template <typename _Iterator, std::uint32_t __radix_bits, typename _ExecutionPolicy, typename _CountBuf>
sycl::event
__radix_sort_scan_submit(_ExecutionPolicy&& __exec, std::size_t __wgroups, _CountBuf& __count_buf,
                         std::size_t __count_buf_size, sycl::event __dependency_event)
{
    // typedefs
    using _CountT = __value_t<_CountBuf>;
    using _SyclPolicy = __decay_t<_ExecutionPolicy>;
    using _KernelName = __radix_sort_scan_kernel<_CountBuf, typename _SyclPolicy::kernel_name>;
    ...
    sycl::event __scan_event = __exec.queue().submit([&](sycl::handler& __hdl) {
        ...
        __hdl.parallel_for<_KernelName>(
            sycl::nd_range<1>(__radix_states, __radix_states), [=](sycl::nd_item<1> __self_item) {
                ...
                for (std::uint32_t __i = 0; __i <= __wgroups; ++__i)
                {
                ...
                std::size_t __total_sum_idx = __wgroups * __radix_states + __self_lidx;
...
template <typename _Iterator, std::uint32_t __radix_bits, bool __is_comp_asc, typename _ExecutionPolicy,
          typename _InBuf, typename _OutBuf, typename _OffsetBuf>
sycl::event
__radix_sort_reorder_submit(_ExecutionPolicy&& __exec, std::size_t __wgroups, std::size_t __block_size,
                            std::uint32_t __radix_iter, _InBuf& __input_buf, _OutBuf& __output_buf,
                            std::size_t __inout_buf_size, _OffsetBuf& __offset_buf, std::size_t __offset_buf_size,
                            sycl::event __dependency_event)
{
    ...
    using _SyclPolicy = __decay_t<_ExecutionPolicy>;
    using _KernelName = __radix_sort_reorder_kernel<_InBuf, _OutBuf, _OffsetBuf, typename _SyclPolicy::kernel_name>;

    // compile kernel to get appropriate sub-group size
    sycl::kernel __k = _KernelName::__compile_kernel(__exec);

    // item info
    // TODO: kernel compilation requires __sg_size to be known at compile-time
    const std::size_t __sg_size = oneapi::dpl::__internal::__kernel_sub_group_size(__exec, __k);
    ...
    sycl::event __reorder_event = __exec.queue().submit([&](sycl::handler& __hdl) {
        ...
        // TODO: find a solution to enforce using max supported SIMD size.
        // (now runtime selects SIMD16 even for devices supporting SIMD32)
        __hdl.parallel_for<_KernelName>(
            sycl::nd_range<1>(__wgroups * __sg_size, __sg_size), [=](sycl::nd_item<1> __self_item) {
                // item info
                const std::size_t __self_lidx = __self_item.get_local_id(0);
                const std::size_t __wgroup_idx = __self_item.get_group(0);
                const std::size_t __start_idx = __blocks_per_wgroup * __block_size * __wgroup_idx + __self_lidx;
                ...
                const std::uint32_t __global_offset_start_idx = __wgroups * __radix_states;
                ...
                for (std::size_t __block_idx = 0; __block_idx < __blocks_per_wgroup * __it_size; ++__block_idx)
                {
                    ...
                    // TODO: investigate the issue with sub_group::exclusive_scan and sub_group::reduce
                    for (std::uint32_t __radix_state_idx = 0; __radix_state_idx < __radix_states; ++__radix_state_idx)
                    {
...
sycl::event
__parallel_radix_sort_iteration(_ExecutionPolicy&& __exec, std::size_t __wgroups, std::size_t __wg_size,
                                std::uint32_t __radix_iter, _InBuf& __in_buf, _OutBuf& __out_buf,
                                std::size_t __inout_buf_size, _TmpBuf& __tmp_buf, std::size_t __tmp_buf_size,
                                sycl::event __dependency_event)
{
    // 1. Count Phase
    sycl::event __count_event = __radix_sort_count_submit<_Iterator, __radix_bits, __is_comp_asc>(
        __exec, __wgroups, __wg_size, __radix_iter, __in_buf, __inout_buf_size, __tmp_buf, __tmp_buf_size,
        __dependency_event);

    // 2. Scan Phase
    sycl::event __scan_event =
        __radix_sort_scan_submit<_Iterator, __radix_bits>(__exec, __wgroups, __tmp_buf, __tmp_buf_size, __count_event);

    // 3. Reorder Phase
    sycl::event __reorder_event = __radix_sort_reorder_submit<_Iterator, __radix_bits, __is_comp_asc>(
        __exec, __wgroups, __wg_size, __radix_iter, __in_buf, __out_buf, __inout_buf_size, __tmp_buf, __tmp_buf_size,
        __scan_event);
...
// TODO: check whether partial_sort_copy fails with -g option because of radix or not
...
__parallel_radix_sort(_ExecutionPolicy&& __exec, _Iterator __first, _Iterator __last)
{
    ...
    // TODO: find better __wg_size [__sg_size .. __max_work_group_size]
    const std::size_t __wg_size = oneapi::dpl::__internal::__max_sub_group_size(__exec);
    // TODO: make it dependent on local memory size
    // the higher the value the less memory size requires from local memory
    const std::size_t __segments = oneapi::dpl::__internal::__max_compute_units(__exec) *
                                   oneapi::dpl::__internal::__max_work_group_size(__exec) * 4;
    const std::size_t __wgroups = std::min(__segments, __get_roundedup_div(__inout_buf_size, __wg_size));
    ...
    const std::size_t __tmp_buf_size = __wgroups * __radix_states + __radix_states;
    ...
    for (std::uint32_t __radix_iter = 0; __radix_iter < __radix_iters; ++__radix_iter)
    {
        ...
        __iteration_event = __parallel_radix_sort_iteration<_Iterator, __radix_bits, __is_comp_asc>(
            __exec, __wgroups, __wg_size, __radix_iter, __in_buf, __out_buf, __inout_buf_size, __tmp_buf,
            __tmp_buf_size, __iteration_event);
