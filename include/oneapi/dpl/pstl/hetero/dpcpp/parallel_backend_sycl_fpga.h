...
template <typename _ExecutionPolicy, typename _Range1, typename _Range2, typename _UnaryOp, typename _BinaryOperation, typename _InitType,
          typename _Inclusive,
          oneapi::dpl::__internal::__enable_if_fpga_execution_policy<_ExecutionPolicy, int> = 0>
auto
__parallel_transform_scan(_ExecutionPolicy&& __exec, _Range1&& __rng1, _Range2&& __rng2, ::std::size_t __n, _UnaryOp __unary_op, _BinaryOperation __binary_op,
                          _InitType __init, _Inclusive)
{
    // workaround until we implement more performant version for patterns
    using _Policy = typename ::std::decay<_ExecutionPolicy>::type;
    using __kernel_name = typename _Policy::kernel_name;
