...
auto
__histogram_general_registers_local_reduction(_ExecutionPolicy&& __exec,
...
template <typename _ExecutionPolicy, typename _Range1, typename _Range2, typename _BinHashMgr>
auto
__backend_impl<oneapi::dpl::__internal::__device_backend_tag>::__parallel_histogram(
    _ExecutionPolicy&& __exec, const sycl::event& __init_event, _Range1&& __input, _Range2&& __bins,
    const _BinHashMgr& __binhash_manager)
{
    if (__input.size() < 1048576) // 2^20
    {
        return __device_backend_details::__parallel_histogram_select_kernel</*iters_per_workitem = */ 4>(
            ::std::forward<_ExecutionPolicy>(__exec), __init_event, ::std::forward<_Range1>(__input),
            ::std::forward<_Range2>(__bins), __binhash_manager);
    }
    else
    {
        return __device_backend_details::__parallel_histogram_select_kernel</*iters_per_workitem = */ 32>(
            ::std::forward<_ExecutionPolicy>(__exec), __init_event, ::std::forward<_Range1>(__input),
            ::std::forward<_Range2>(__bins), __binhash_manager);
    }
}
