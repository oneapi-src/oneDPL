...
struct __parallel_merge_submitter_large_V1<_IdType, __internal::__optional_kernel_name<_Name...>>
{
  protected:
    ...
    __calculate_split_points(_ExecutionPolicy&& __exec, _Range1&& __rng1, _Range2&& __rng2, _Compare __comp) const
    {
        ...
        const auto __main_diag_offset = oneapi::dpl::__internal::__dpl_ceiling_div(__n, 2);                             // 12'750'000
        const auto __nearest_to_main_diagonal_offset = oneapi::dpl::__internal::__dpl_bit_floor(__main_diag_offset);    //  8'388'608
        const auto __iterations = 2 * sycl::ctz(__nearest_to_main_diagonal_offset);                                     //         23
        ...
        const auto __half_of_diagonals_count = oneapi::dpl::__internal::__dpl_ceiling_div(__diagonals_count, 2);        // 12'750

        // In result storage we save iteration (diagonal no devisor) number
        __split_points_scratch_storage_t<_ExecutionPolicy> __split_points(__exec, 1, __diagonals_count);
        ...
        // The multiplicity of the processing diagonal number (aka __linear_id)
        __iteration_t __iteration_state = __half_of_diagonals_count;
        ...
        {   // for sycl::buffer scope
            ...
            sycl::buffer<__iteration_t, 1> __iteration_state_buffer (&__iteration_state,  __range_1);
            ...
            __event = __exec.queue().submit([&](sycl::handler& __cgh) {
                ...
                // Get access to iterations number
                auto __result_acc = __split_points.__get_result_acc(__cgh);
                auto __result_ptr = __split_points.__get_usm_or_buffer_accessor_ptr(__result_acc);

                // Get access to the split points
                auto __split_points_acc = __split_points.__get_scratch_acc(__cgh);
                auto __split_points_ptr = __split_points.__get_usm_or_buffer_accessor_ptr(__split_points_acc);

                auto __iteration_state_buffer_acc  = __iteration_state_buffer.get_access<sycl::access::mode::read_write>(__cgh);
                ...
                __cgh.parallel_for<_FindSplitPointsKernel>(
                    ...
                    [=](sycl::item</*dim=*/1> __item_id) {
                        ...
                        __itertations_atomic_t __iteration_ref       (__iteration_state_buffer_acc [0]);
                        __itertations_atomic_t __data_initialized_ref(__data_initialized_buffer_acc[0]);

                        // Common data initialization
                        if (__linear_id == 0 && __data_initialized_ref.load() == 0)
                        {
                        ...
                        {
                            while (__data_initialized_ref.load() == 0)
                            {
                                // Do nothing
                            }
                        }

                        const __iteration_t __iteration_ref_loaded = __iteration_ref.load();

                        // The number of the processing diagonal is a multiple of the iteration number
                        // We did not process this diagonal in the previous iterations
                        if (0 < __linear_id && __linear_id % __iteration_ref_loaded == 0/*
                            && __linear_id * 2 % __iteration_ref_loaded != 0*/)
                        {
                            const _IdType __diagonal_offset = __linear_id * __diagonals_interval;

                            __split_point_t __split_point;
                            if (__item_id % __half_of_diagonals_count == 0)
                            {
                                __split_point = __find_start_point(__rng1, __rng2, __diagonal_offset, __n1, __n2, __comp);
                            }
                            else
                            {
                                const _IdType __rng1_from = __item_id < __half_of_diagonals_count ? 0 : __split_points_ptr[__half_of_diagonals_count].first;
                                const _IdType __rng1_to   = __item_id < __half_of_diagonals_count ? __split_points_ptr[__half_of_diagonals_count].first : __n1;
                                const _IdType __rng2_from = __item_id < __half_of_diagonals_count ? 0 : __split_points_ptr[__half_of_diagonals_count].second;
                                const _IdType __rng2_to   = __item_id < __half_of_diagonals_count ? __split_points_ptr[__half_of_diagonals_count].second : __n2;

                                __split_point = __find_start_point_in(__rng1, __rng1_from, __rng1_to,
                                                                        __rng2, __rng2_from, __rng2_to,
                                                                        __diagonal_offset,
                                                                        __n1, __n2,
                                                                        __comp);
                            }
                        ...
                        if (__linear_id > 0)
                        {
                            // We are waiting for the first work-item to change the iteration number.
                            while (__iteration_ref.load() == __iteration_ref_loaded)
                            {
                                // Do nothing.
                            }
                        }
                        ...
                        {
                            __iteration_ref.store(__iteration_ref_loaded / 2);
                        }
                    });
            });
        }   // for sycl::buffer scope
        ...
        auto __event = __exec.queue().submit([&](sycl::handler& __cgh) {
            ...
            __cgh.parallel_for<_Name...>(
                ...
                [=](sycl::item</*dim=*/1> __item_id) {
                    ...
                    {
                        // TODO how to define correct __chunk value here?
                    }
