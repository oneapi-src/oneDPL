...
struct __subgroup_radix_sort
{
    ...
    operator()(sycl::queue __q, _RangeIn&& __src, _Proj __proj)
    {
        ...
        using _SortKernelLocReqdSgSz = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
            __radix_sort_one_wg_kernel<_KernelNameBase, __wg_size_t, __block_size_t, __call_0_t, __call_0_t>>;
        using _SortKernelGlobReqdSgSz = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
            __radix_sort_one_wg_kernel<_KernelNameBase, __wg_size_t, __block_size_t, __call_1_t, __call_0_t>>;
        using _SortKernelGlobReqdSgSz = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
            __radix_sort_one_wg_kernel<_KernelNameBase, __wg_size_t, __block_size_t, __call_1_t, __call_0_t>>;
        using _SortKernelLocNoReqdSgSz = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
            __radix_sort_one_wg_kernel<_KernelNameBase, __wg_size_t, __block_size_t, __call_0_t, __call_1_t>>;
        using _SortKernelGlobNoReqdSgSz = oneapi::dpl::__par_backend_hetero::__internal::__kernel_name_provider<
            __radix_sort_one_wg_kernel<_KernelNameBase, __wg_size_t, __block_size_t, __call_1_t, __call_1_t>>;

        using _KeyT = oneapi::dpl::__internal::__value_t<_RangeIn>;
        bool __can_use_slm = __check_slm_size<_KeyT>(__q, __src.size());
        bool __can_use_tuned_sg_size = __check_sg_size(__q);
        //check SLM size
        if (__can_use_slm && __can_use_tuned_sg_size)
            return __one_group_submitter<_SortKernelLocReqdSgSz>()(__q, ::std::forward<_RangeIn>(__src), __proj,
                                                                   std::true_type{} /*SLM*/, std::true_type{} /*REQD SG SZ*/);
        else if (!__can_use_slm && __can_use_tuned_sg_size) 
            return __one_group_submitter<_SortKernelGlobReqdSgSz>()(__q, ::std::forward<_RangeIn>(__src), __proj,
                                                                    std::false_type{} /*No SLM*/, std::true_type{} /*REQD SG SZ*/);
        else if (__can_use_slm && !__can_use_tuned_sg_size) 
            return __one_group_submitter<_SortKernelLocNoReqdSgSz>()(__q, ::std::forward<_RangeIn>(__src), __proj,
                                                                     std::true_type{} /*No SLM*/, std::false_type{} /*REQD SG SZ*/);
        else
            return __one_group_submitter<_SortKernelGlobNoReqdSgSz>()(__q, ::std::forward<_RangeIn>(__src), __proj,
                                                                      std::false_type{} /*No SLM*/, std::false_type{} /*REQD SG SZ*/);
    }
  ...
  private:
    ...
    bool
    __check_sg_size(sycl::queue __q)
    {
        auto __sup_sub_group_szs = __q.get_device().get_info<sycl::info::device::sub_group_sizes>();
        return ::std::find(__sup_sub_group_szs.begin(), __sup_sub_group_szs.end(), __req_sub_group_size)
                    != __sup_sub_group_szs.end();
    }
    ...
    {
      private:
        template <typename _NDItem, typename _RangeIn, typename _Proj, typename _ExchangeLacc, typename _CountLacc>
        static void __one_group_kernel_impl(_NDItem __it, uint16_t __n, _RangeIn&& __src,
            _Proj __proj, _ExchangeLacc __exchange_lacc, _CountLacc __counter_lacc)
        {
...
public:
        template <typename _RangeIn, typename _Proj, typename _SLM_tag, typename _Reqd_sg_sz>
        auto
        operator()(sycl::queue __q, _RangeIn&& __src, _Proj __proj, _SLM_tag, _Reqd_sg_sz)
        {
            ...
            return __q.submit([&](sycl::handler& __cgh) {
                ...
                if constexpr (_Reqd_sg_sz::value)
                {
                    __cgh.parallel_for<_Name...>(
                        __range, ([=](sycl::nd_item<1> __it)[[_ONEDPL_SYCL_REQD_SUB_GROUP_SIZE(__req_sub_group_size)]] {
                        __one_group_kernel_impl(__it, __n, __src, __proj, __exchange_lacc, __counter_lacc);
                    }));
                }
                else
                {
