...
__pattern_reduce_by_segment(_ExecutionPolicy&& __exec, _Range1&& __keys_in, _Range2&& __vals_in, _Range3&& __keys_out,
                            _Range4&& __vals_out, _BinaryPredicate&& binary_pred, _BinaryOperator&& __binary_op)
{
    ...
    auto __res = std::copy_if(
        ::std::forward<_ExecutionPolicy>(__exec), __iota.begin(), __iota.end(), __idx, [&__keys_in, __n](auto __i) {
            return __i == 0 || __i == __n || !__binary_predicate(__keys_in[__i], __keys_in[__i - 1]);
        });
    ...
    ::std::for_each(::std::forward<_ExecutionPolicy>(__exec), __v.begin(), __v.begin() + __v.size(),
                    [&__keys_in, &__vals_in, &__keys_out, &__vals_out, &__idx, __binary_op, __exec](auto i) {
                        ...
                        __vals_out[i] = reduce(::std::execution::seq, __vals_in.begin() + __idx[i],
                                               __vals_in.begin() + __idx[i + 1], __binary_op);
                    });
