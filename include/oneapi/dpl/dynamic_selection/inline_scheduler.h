...
struct int_inline_scheduler_t {
    ...
    void wait_for_all() override {
      if (wait_reported_->exchange(true) == false) {
        if constexpr (PropertyHandle::should_report_task_completion) {
          ds::property::report(p_, ds::property::task_completion);
        }
        if constexpr (PropertyHandle::should_report_task_execution_time) {
          ds::property::report(p_, ds::property::task_execution_time, w_);
        }
      }
    }
  };
  ...
  auto submit(SelectionHandle h, Function&& f, Args&&... args) {
    ...
    if constexpr (PropertyHandle::should_report_task_submission) {
      ds::property::report(h.get_property_handle(), ds::property::task_submission);
    }
  ...
  universe_container_t query(ds::property::universe_t) const noexcept {
    return universe_;
  }
  ...
  auto report(ds::property::universe_t, const universe_container_t &u) noexcept {
    universe_ = u;
  }
