...
float dpstd_native3(size_t num_steps, int groups, Policy&& policy) {
  ...
  auto calc_begin = dpstd::begin(buf);
  auto calc_end = dpstd::end(buf);

  using Functor = dpstd::unseq_backend::walk1<Policy, my_no_op>;
  float result;
  ...
  auto tf_init =
      dpstd::unseq_backend::transform_init<Policy, std::plus<float>, Functor>{
          std::plus<float>(), Functor{my_no_op()}};

  auto combine = std::plus<float>();
  auto brick_reduce =
      dpstd::unseq_backend::reduce<Policy, std::plus<float>, float>{
          std::plus<float>()};
...
float dpstd_native4(size_t num_steps, int groups, Policy&& policy) {
  ...
  auto calc_begin = dpstd::begin(buf2);
  auto calc_end = dpstd::end(buf2);

  using Functor2 = dpstd::unseq_backend::walk1<Policy, slice_area>;
  ...
  // rectangle.  
  auto tf_init =
      dpstd::unseq_backend::transform_init<Policy, std::plus<float>, Functor2>{
...
float dpstd_two_steps_lib(int num_steps, Policy&& policy) {
  ...
  std::transform(policy, dpstd::counting_iterator<int>(1),
                 dpstd::counting_iterator<int>(num_steps), calc_begin2,
                 [=](int i) {
...
float dpstd_onestep(int num_steps, Policy& policy) {
  ...
  float total = std::transform_reduce(
      policy, dpstd::counting_iterator<int>(1),
      dpstd::counting_iterator<int>(num_steps), 0.0f, std::plus<float>(),
      [=](int i) {
